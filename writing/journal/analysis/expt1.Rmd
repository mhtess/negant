...
fig.path <- "~/Documents/talks/langcog-0219/figs/"
...
d.l1.7.comments %>% select(workerid, enjoyment, comments) %>% kable()
```
...
# maximal model
## (adding + antonym_type by item fails to converge)
rs.7.helmert2 <- lmer(response ~ antonym_type * st + 
                       (0 + antonym_type:st | workerid) + 
                       (0 + antonym_type:st | positive),
                     data = d.l1.natEng)
summary(rs.7.helmert)
```
```{r}

rs.7.helmert.brm <- brm(response ~ antonym_type * st + 
                       (1 + st | workerid) + 
                       (1 + st | positive),
                     data = d.l1.natEng,
...
summary(rs.7.helmert.brm)


rs.7.helmert.brm.max2 <- brm(response ~ antonym_type * st + 
                       (1 + st + antonym_type | workerid) + 
                       (1 + st + antonym_type| positive),
                     data = d.l1.natEng,
                    chains = 3, cores = 3, iter = 2000,
                    warmup = 1000)
summary(rs.7.helmert.brm.max2)
