...
```{r runSingle}
rs.listener.test <- webppl(paste(rsaBinsCoarserGauss,
                                 utils, 
                                 uncertainHasThresholdsRSA,
                                 #'listener1(["not_unhappy"])',
                                 sep = '\n'),
...
rs.listener.test %>%
  select(-state.not_unhappy) %>%
  group_by(compositional_un, compositional_not) %>%
  summarize(probs = sum(prob))

# if the cost is equal, the Uncertain Neg model still prefers a compositional "not" and non compositional "un" to the alternative (though overall there is a preference for both being noncompositional)
