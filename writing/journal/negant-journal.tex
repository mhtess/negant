...
%\title{When two negatives don't make a positive}
...
\abstract{
One useful function of communication is to convey the opposite of a proposition, but natural languages provide multiple ways of stating the opposite: A person who is \emph{not happy} might be \emph{unhappy}, \emph{sad}, or perhaps neither, just \emph{not happy}. Intuition suggests, contra standard logic, that two negatives do not make a positive but rather create nonredundant meanings: A person \emph{not unhappy} is not exactly \emph{happy}.
We hypothesize this behavior stems from listeners entertaining multiple opposite meanings for negation markers like \emph{not} and \emph{un-}; then, a speaker who uses two negation provides a clear signal that they intend the negations in a nonredundant manner.
...
We formalize this hypothesis in a computational model of language understanding and show that double negations are predicted to have more subtle meanings than the simple positive expressions.
The uncertain negation hypothesis also makes surprising additional predictions about single negations conveyed in distinct manners (\emph{unhappy}~vs.~\emph{not happy}): The two are predicted to have identical meanings when presented in isolation, but when a speaker uses both in the same context, \emph{unhappy} should be more negative than \emph{not happy}. 
Finally, the model predicts that blatant double negations (ones that use same negation marker twice e.g., \emph{not not happy}) should too have nonredundant meanings. 
Three behavioral experiments confirm consistent orderings of interpretations that interact with the presentational context in the way predicted by the model. 
...
Language reflects thought, but we think and feel in ways that are not always easily expressible in words.
...
In other words, \enquote{not unhappy} (a \emph{negated antonym}) could indicate a slightly positive state, below that of \enquote{happy}  and perhaps not neutral. %, where feelings that are neither positive nor negative reside. 
...
Also, insofar as nonredundent meanings can be expressed through double negations, such language could provide new avenues for understanding the language of emotions, like how it feels to be \emph{not unhappy} \cite{Lindquist2015, Satpute2016}. Moreover, a better understanding of such phenomena will provide deeper insight into how we manipulate and contort language to navigate and communicate the gradations of our conceptual landscape. 
...
As described, this analysis indicates only that \enquote{not unhappy} conveys neutral feelings: \emph{neither happy nor unhappy}, contra the intuition expressed by \citeA{Jespersen1924} that \enquote{not unhappy} indicates a slightly positive state.
It is plausible that listeners entertain other possibilities regarding what the speaker could have said: The speaker could have said single negations (\enquote{not happy} or \enquote{unhappy}; \cite{Blutner2004:pragmatics}).
Introducing new alternatives, however, only raises more questions: Are \enquote{unhappy} and \enquote{not happy} different in meaning?
\cite{Jespersen1917:Negation} and \cite{Blutner2004:pragmatics} believe the two expressions have the same meaning, while \citeA{Krifka2007:Negated-antonyms} disagrees, citing examples like:
...
A positive integer must be either even or odd, because even and odd are \emph{contradictory} opposites: They cannot both be true but they also cannot both be false. 
...
Contradictory opposition can be iterated (e.g., a number that is not not even is even) while contrary opposition cannot (e.g., a building that is not not tall is likely not tall).\footnote{
...
Because contraries do not iterate, the only logical possibilities for the whole expression is a double contradiction or a contradiction about a contrary: A double contradiction carries the same meaning as a positive adjective (\emph{happy}) and thus a rational speaker would not have bothered to say \emph{not unhappy} (a more complex expression) if a double contradiction was their intention, so they likely were contradicting a contrary.
