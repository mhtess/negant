...
%\authornote{Blinded for review}
\authornote{manuscript under review.}
%\abstract{
...
When common speech is sufficient to express subtle feelings, speakers resort to creative language \cite<e.g., metaphors;>{lakoff2008} or uncommon, perhaps novel terms like being ``plateaued'' \cite{bardwick1986plateauing} or residing in a ``zone of indifference'' \cite{sapir1944grading}.
There are systematic ways of expressing subtle gradations as well, such as with constructions involving double negations: A person \emph{not unhappy} is probably not entirely happy.
The interpretation of double negatives, in particular, is important from a logical perspective \cite{Horn1989:Natural, Krifka2007:Negated-antonyms} as well as a legal one, where such constructions are surprisingly common \cite{tiersma1999legal}.
%Also,
...
More broadly, pinning down the meaning of negation---one of the most logical elements of language---is crucial for much of psychological science that depends upon linguistic information to convey task instructions and content \cite<e.g., in the psychology of reasoning;>{geurts2003reasoning, lassiter2015many} 
...
These intuitions are subtle, not universally agreed upon, and further complicated by the fact that the meaning of double negations (\emph{not unhappy})  invariably depends upon the meaning of the component, single negations (\emph{not}, \emph{un-}), which are also difficult to pin down. % are also not obviously different from one another.
...
\caption{Alternative semantic hypotheses. A: Logical relations among negation markers under three different models of meaning. B: Literal meanings of antonym quartets on a happiness scale under three different models.  \label{fig:happy-scale}}
\end{figure}

\begin{figure}[t]
\centering \includegraphics[width=0.8\textwidth]{figs/schematicMeanings}  
\caption{The \ourmodel model reasons over a hypothesis space of meanings for antonym pairs and their negations.
...
Ambiguity is ubiquitous in natural language and seldom harms communication.
Interlocutors rely on context to reason about each others' beliefs and goals to retrieve the intended meaning of underspecified words \cite{Grice1975, clark1996using, levinson2000presumptive}.
...
Theorizing about the meanings of negation must consider how the communicative context supports and potentially alters meanings, but heretofore no formal models have been developed that can adequately address the puzzles of double negatives while also accounting for pragmatic reasoning.
%
We therefore investigate the question of the meaning of natural language negation by formalizing the semantic proposals---Aristotelean, Orwellian, \ourmodel---in models of pragmatic communication that assume listeners interpret utterances as rational social actions. 

We take the Rational Speech Act framework as a common, pragmatic reasoning core to relate the different semantic proposals to pragmatically-enriched interpretations that can be compared to human behavioral data \cite{Franke2015a, Goodman2016:RSA, problang}.
The different semantic hypotheses define a lexicon $\mathcal{L}$ to be used inside of a probabilistic model of pragmatic reasoning, wherein a pragmatic listener $L_1$ resolves the meaning of an utterance $u$ by reasoning about why a rational speaker $S_1$ would have bothered to produce said utterance.
Formally, a scalar adjective $H$ (e.g., \emph{happy}) is literally true when the degree associated with that adjective \(x\) (e.g., the degree of happiness) is greater than some contextually-determined threshold \(\theta\) \cite{Kennedy2007}: $\mbox{ $[\![H]\!]$}(x, \theta): x > \theta$.
Contradictory negation is the truth-value swapping operation well-known from classical logic: $\mbox{ $[\![\neg H]\!]$}(x, \theta): x \leq \theta$.
Contrary negation, on the other hand, is a predicate-forming operation that introduces a new distinct threshold \(\theta'\): $\mbox{ $[\![ \tilde{H} ]\!]$}(x, \theta'): x < \theta'$.

Pragmatic reasoning about the truth-conditional threshold $\theta$ can be used to derive vague, context-sensitive, intuitively plausible interpretations of gradable adjectives (e.g., ``a tall boy''~vs.~``a tall basketball player'') \cite{Lassiter2015}.
Our Aristotelean model extends this basic idea by assuming that \emph{happy} and \emph{unhappy} are contraries , whose thresholds $\theta$ and $\theta'$ are jointly inferred. 
The probabilistic pragmatics model of Aristotelean contraries is given by:
%
\begin{align}
...
The pragmatic listener (Eq.~\ref{eq:L1aris}) interprets an utterance $u$ by reasoning about two variables: the intended meaning (or, degree of happiness) $x$ and the truth-conditional thresholds $\theta$ and $\theta'$.  
...
Following \citeA{Lassiter2015}, the prior distribution on thresholds is also defined to be a uniform distribution over the unit interval $P(\theta) = \text{Uniform}(0, 1)$. 
The listener resolves their uncertainty by reasoning about the behavior of a speaker (Eq.~\ref{eq:S1}), a soft-max rational agent (with degree of rationality  $\alpha$) who aims to act in accordance with a standard, information-theoretic utility function that tracks how well an utterance conveys the speaker's intended meaning $x$ (i.e., the degree of happiness) to this literal listener---$\ln {L_{0}(x \mid u, \theta, \mathcal{L}}$)---while taking into account the cost of the utterance---$\text{cost}(u)$.
...
We assume in our models that the set of alternative utterances contains the positive adjective, the antonym, and their corresponding particle negations (e.g., happy, unhappy, not happy, not unhappy). 
We further assume the cost of morphological negation is less than the cost of particle negation (i.e., $cost(un) < cost(not)$) and that costs are additive (i.e., $cost(not\text{ }un) = cost(un) + cost(not)$), though we examine model predictions under different assumptions of cost in the SI.
...
The Aristotelean model produces the intuitive ordering of interpretations hypothesized by \citeNP{Krifka2007:Negated-antonyms} and others: A person \emph{unhappy} is less happy than someone (merely) \emph{not happy}; someone \emph{not unhappy} is in a slightly positive state, above the midpoint of happiness but below that of \emph{happy}  (Figure \ref{fig:modelPredictions}).
These predictions result from the semantics of contraries~vs.~contradictions and their interaction with the vagueness of gradable adjectives: these predictions do not result from a model of pragmatics that operates over only contradictory negations (the George Orwell model) nor from an Aristotelean model in which the predicates have fixed, not vague, meanings (see SI). 
The ordering (\emph{unhappy} $<$ \emph{not happy} $<$ \emph{not unhappy} $<$ \emph{happy}), while intuitive, has not been empirically validated, and some theorists attest to only a partial ordering wherein  \emph{not happy} $\approx$ \emph{unhappy} \cite{Jespersen1917:Negation, Blutner2004:pragmatics}. Thus, we develop an alternative hypothesis by introducing flexibility into what negation operators can mean.

%Some argue that single negations have the same meaning \cite<e.g., \emph{not happy} = \emph{unhappy};>{Jespersen1917:Negation, Blutner2004:pragmatics},



%The Orwellian model is given by a simplified form of Eq.~\ref{eq:L1aris}, which omits representations of $\theta'$ since all negations are assumed to be contradictions; this model is useful to isolate the impact of pragmatic reasoning on vague adjective interpretations under a strict model of negation (only contradictions). 
%The details of this lexicon is what differs between the models.

The \ourmodel model extends the Aristotelean model  by introducing uncertainty into the pragmatic listener about how their interlocutor (speaker $S_1$) uses negation markers (``not'', ``un-'') to convey contradictory~vs.~contrary negations \cite{Bergen2016}.
...
The other components of the model ($S_1$ and $L_0$) are the same as Eqs.~\ref{eq:S1} \& \ref{eq:L0}.


The \ourmodel model predicts a pattern of data that is unique from that of the Aristotelean and Orwellian models (Figure \ref{fig:modelPredictions}).
When utterances involving a single negation marker are heard in isolation (e.g., \emph{not happy} or \emph{unhappy}), they are interpreted identically because of the ambiguity in whether each maps onto a contrary or contradiction (Figure \ref{fig:meanings}).  
If the listener encounters a double negation (e.g., \emph{not unhappy}), however, pragmatic reasoning helps disambiguate that the speaker likely intended a contradiction of a contrary (\emph{a la} the Aristotelean account) because it would otherwise be a very costly manner of expressing the same meaning as the positive adjective (\emph{happy}), which would result if both negation markers conveyed contradictions. % from a double contradiction.

The disambiguating power of hearing multiple negations (\emph{[not] [un-]happy}) can also be observed for single negations (\emph{not happy}, \emph{unhappy}) if the listener hears (or, intuitively considers) multiple distinct utterances in the same context (e.g., \citeNP{Krifka2007:Negated-antonyms}'s example, or simply: ``She's not happy. He's unhappy.''; Figure \ref{fig:modelPredictions}).
These predictions are generated by conditioning each model on the observation of a speaker using all four adjective alternatives $u'$ to describe different referents $x'$ (e.g., \emph{Sue is happy. Steve is not happy. Bill is unhappy. Barb is not unhappy.}; Figure \ref{fig:modelPredictions}, \emph{multiple utterances}):
%
\begin{align}
...
\end{align}
%
These indirectly contrastive inferences result from the fact that the listener has more evidence that the speaker associates different meanings with the different negation markers.
The \ourmodel model interprets the morphological antonym (\emph{unhappy}) as more strongly negative than the utterance involving a negation particle (\emph{not happy}) as a result of a cost difference between morphological and particle negation (e.g., with a word-based cost function: morphological markers do not add a new word to the utterance whereas negation particles do;  see SI for further discussion of modeling assumptions).
...
%Our models combine previous, independently established modeling proposals on pragmatic reasoning \cite{Franke2015a, Goodman2016:RSA, problang}, the interpretation of gradable adjectives \cite<e.g., \emph{tall}, \emph{happy};>{Kennedy2007, Lassiter2015}, and ambiguity in the meanings of words \cite{Bergen2016}. 
%
%
%
%
...
%
%Our models are of listeners who hear utterances (e.g., \emph{not unhappy}) and compute a posterior distribution over degrees of happiness, representing predictions about the interpretation of an utterance (Figure \ref{fig:modelPredictions}), which we test in the experiments that follow.

%We compare these distributions from each model to empirical judgments in three experiments.

%We describe the key differences of these hypotheses that lead to different predictions, but each hypothesis is supported by a completely formalized computational model that can be found in the SI. 
...
All data, analysis script, experimental materials, and models can be found at \texttt{blinded for review}.
%\url{https://osf.io/4gsmd/}. 
...
We use the lexical antonyms as a control representing how antonyms should behave under the Aristotelean account. Thus, the key prediction is an interaction to see whether the antonym~vs.~negated positive contrast (\emph{unhappy} vs. \emph{not happy}) is different for morphological antonyms than it is for lexical antonyms, with morphological antonyms predicted to show no difference while lexical antonyms are predicted to show a difference.
...
%}
Consistent with our predictions, lexical antonyms (short) were interpreted more negatively than their associated negated positive (not tall) in comparison to the morphological antonyms (happy) and their negated positives (not happy) the difference between the \emph{antonym} vs.~\emph{negated positive} 
...
Investigating the distribution of responses more closely, we see that negated antonyms received a distinct bimodal distribution wherein most ratings were slightly positive but a minority distribution of ratings were slightly negative (e.g., \emph{not dishonest} meaning \emph{not honest}).
This weakly negative interpretation for negated antonyms was present at least somewhat in every item and in most participants (see SI for item-wise plots).
This interpretation may be the result of participants attributing politeness to the speaker; when a speaker cares about a listener's self-image, they tend to endorse utterances with negation:  \emph{Not dishonest} may be an indirect way of saying that a person is not honest \cite{Yoon2017}.
...
In this antonym elicitation, participants saw the same materials as in the main experiment (e.g., ``Your friend tells you about their friend: William. \emph{William is forgiving.}'') and asked \enquote{What is the opposite of \emph{[adjective]}?} (``What is the opposite of forgiving?'').
By asking participants for the ``opposite'', we hoped to generate adjectives that conveyed the opposite polarity but with the same intensity as the positive adjective (e.g., the opposite of happy is ``sad'' as opposed to ``devastated'' or ``miserable'', which are intuitively more negative than ``happy'' is positive).
From the list of freely-produced opposites, the first author chose the one that intuitively best conveyed the same scalar dimension as the morphological antonym and which was not already used as a lexical antonym for another item (e.g., opposite of \emph{forgiving} \(\rightarrow\) \emph{resentful}; opposite of \emph{kind} \(\rightarrow\) \emph{cruel}; opposite of \emph{friendly} \(\rightarrow\) \emph{mean}).
...
Pragmatics in particular is a complicated, elusive, and often-ignored factor that is necessary to consider to understand how the meanings of words are computed in context. 
...
These findings highlight just how deep the flexibility and context-sensitivity of language run and underscore the importance of discarding simplistic pictures of language understanding that assume transparent and static meanings of words---even logical words. 
...
The equivalent interpretations of morphological antonyms and negated positives could potentially arise via a different mechanism than the \ourmodel model.
If a speaker says they are ``not good'' when they could also have said they are ``fine'', an implication may be that they are rather bad \cite<so-called ``negative strengthening'';>{Horn1989:Natural, Ruytenbeek2017, Gotzner2018}.
% process of \emph{negative strengthening}, wherein a negated positive adjective receives a more negative interpretation than literally implied \cite<e.g., \emph{not intelligent} $\rightarrow$ rather stupid;>{Horn1989:Natural, Ruytenbeek2017, Gotzner2018}. 
This alternative pragmatic hypothesis has never been articulated formally, however, and extending the Aristotelean model with a midpoint-denoting neutral utterance (e.g., ``fine'') introduces new problems: The interval semantics for the neutral utterance leads to an interpretation for ``not happy'' that includes highly positive statements (i.e., not extremely happy but not just fine; see SI).
Articulating the other pragmatic components necessary to derive the inferences described by \citeA{Horn1989:Natural} is an important area for future research.
...
We presented a computational solution to an age-old problem in natural language understanding: How to interpret double negatives \cite<e.g., \emph{not unhappy}; >{Horn1991:Duplex, Krifka2007:Negated-antonyms, Rett2014:eval}.
We predicted and observed empirically the ordering hypothesized by \citeA{Krifka2007:Negated-antonyms} for morphological antonyms (\emph{unhappy} $<$ \emph{not happy} $<$ \emph{not unhappy} $<$ \emph{happy}), when a listener hears multiple adjectival utterances in the same context.
...
%This work thus carries with it an account of a robust linguistic intuition: 
%Potentially equivalent utterances receive differential interpretations when produced by the same speaker in close proximity, an instance of \emph{mutual exclusivity} reasoning \cite{Markman1989}. 
%\mht{are the other lexical uncertainty papers not an instance of this general point as well?}
...
In Experiment 3, we saw that flexible pragmatic interpretation goes beyond using distinct negation markers: Expressions that use the same negation marker twice (e.g., \emph{not not happy}) are interpreted in a manner distinct from that of positive adjectives (e.g., \emph{happy}) and that of negated antonyms (e.g., \emph{not unhappy}). 
This result points to an underlying cognitive mechanism that flexibly allows for different meanings for the same negation marker within a single utterance.

Our work builds upon previous studies on negated adjectives \cite<e.g.,>{giora2005negation} with our straight-forward response measurements, comparison of morphological and lexical antonyms, and situation within different presentational contexts.
...
\cite{Ruytenbeek2017, Gotzner2018} to which we provide a new way of measuring the equivalence between negated positives and antonyms.
Our results suggest, however, that in the absence of further context, morphological antonyms (\emph{unhappy}) are interpreted more like contradictions than negated positives  (\emph{not happy}) as contraries.
...
Our finding that negated antonyms are interpreted more negatively than positive adjectives is also consistent with negated antonyms  (e.g., \emph{not impossible})  being processed more easily then positive adjectives (e.g., \emph{possible}) when the listener expects the degree to be low \cite<something very likely to difficulty;>{schiller2017solving}.
...
The simple observation that negation particles (``not'') are more likely under polar QUDs (\emph{is John happy?}), however, is insufficient to explain our data: It does lead to ``not happy'' being interpreted similarly to ``unhappy'', but at the cost of washing out the difference between ``not unhappy'' and  ``happy'' (see SI).
QUDs are likely to play a role in understanding negation and the modeling approach we present here provides a framework for elaborating more complex hypotheses about the relationship between the QUD and utterances involving negation.
...
This flexibility in interpretation might be elicited by our text-based experiment, which leaves open exactly how our stimuli should sound to the ear; prosodic focus can more strongly constrain interpretations, which will be important to clarify using speech-based experiments. 
...
%Though our experiments were conducted in English, we expect these patterns to hold cross-linguistically for languages without negative concord and with negation markers as productive as English \emph{not} and \emph{un-}. 
...
%a recent electrophysiological processing study which found that negated antonyms (e.g., \emph{not impossible}) were processed more easily than positive adjectives (e.g., \emph{possible}) when participants had prior expectations that the degree was low \cite<e.g., something was very likely to difficulty;>{schiller2017solving}.

Our findings and modeling suggest that even seemingly rigid linguistic elements like negation have some kind of ambiguity in their meaning.
...
Formalizing the structure of this ambiguity and how listeners reason about it is an important step to understanding the complexities that reside deep in human language. 

%Listeners may also reason about ambiguity even when speakers do not intend.
%Speaker intentions may also be 
