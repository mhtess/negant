...
\abstract{
Logic tells us that two negatives make a positive, but in natural language, things are not so black and white: A person \emph{not unhappy} is not entirely \emph{happy}, though is possibly more happy than unhappy.
We hypothesize the nonredundancy of double negatives stems from listeners entertaining multiple opposite meanings for negation markers like \emph{not} and \emph{un-}, which we formalize in a computational model of language understanding. %; then, a speaker who uses two negations signals that she intends the negations in a nonredundant manner.
...
In addition to predicting nonredundant meanings for double negatives, this uncertain negation hypothesis makes surprising additional predictions about single negations (\emph{unhappy}~vs.~\emph{not happy}): The two are predicted to have identical meanings when presented in isolation, but not when a speaker uses both in the same context. %, \emph{unhappy} should be more negative than \emph{not happy}. 
