...
We investigate the question of the meaning of natural language negation markers by formalizing the semantic proposals---Aristotelean, Orwellian, \ourmodel---in models of pragmatic communication that assume listeners interpret utterances as rational social actions. 
...
Our models combine previous, independently established modeling proposals on pragmatic reasoning \cite{Franke2015a, Goodman2016:RSA, problang}, the interpretation of gradable adjectives \cite<e.g., \emph{tall}, \emph{happy};>{Kennedy2007, Lassiter2015}, and ambiguity in the meanings of words \cite{Bergen2016}. We describe the key qualitative differences of these models that lead to different predictions, but each model is supported by a completely formalized language understanding model, the mathematical details of which can be found in the SI. 
Each model takes in an utterance (e.g., \emph{not happy}, \emph{not unhappy}) and returns a posterior distribution over degrees of happiness, representing the listener's beliefs / interpretation (Figure \ref{fig:modelPredictions}), which we then qualitatively compare to empirical judgments in three experiments to determine which model most accurately characterizes the behavioral response patterns observed in human participants. 
...
The \ourmodel model interprets the morphological antonym (\emph{unhappy}) as more strongly negative than the utterance involving a negation particle (\emph{not happy})  as a result of a cost difference between morphological and particle negation: we assume the production cost of morphology is less than that of particle negation because the speaker produces a one-word utterance as opposed to a two-word utterance. \mf{maybe add something like: ``see SI for more discussion of our modeling assumptions'' if that's what we have in the SI}
...
We pre-registered the exclusion criterion of removing participants who failed to respond correctly to at least 7 of the 10 items. \mf{I moved this from main text to footnote; maybe shorten footnote?}}
...
In Experiments 1 \& 2, we discovered and confirmed a surprising empirical result: \emph{unhappy} (morphological antonyms) and \emph{not happy} (negated positives) are interpreted identically, except when heard in the same context.
One explanation for this result is via a process known as \emph{negative strengthening}, where a negated positive adjective receives a stronger interpretation than literally implied \cite<e.g., \emph{not intelligent} $\rightarrow$ rather stupid;>{Horn1989:Natural, Ruytenbeek2017, Gotzner2018}. 
This pragmatic, negative strengthening explanation has never been articulated formally and so it  is not clear what pragmatic components are necessary to derive this inference. 
It is classically explained via alternative-based pragmatic reasoning: A speaker could have they are ``fine'' and the fact that they chose to say they not ``not good'' implies that they are rather bad \cite{Horn1989:Natural}. 
In the SI, we present two versions of a pragmatic reasoning model that introduces competition with a neutral utterance (``fine'') and find that it does not capture the key behavioral patterns we observe.
...
In Experiment 3, we discovered that this inference goes beyond using distinct  negation markers: Expressions that use the same negation marker twice (e.g., \emph{not not happy}) are interpreted a manner distinct from that of positive adjectives (e.g., \emph{happy}) and that of negated antonyms (e.g., \emph{not unhappy}). 
...
The interpretations we observe empirically for negated antonyms on average indicate a slightly positive state, but we observe consistent  responses indicating a slightly negative interpretation as well (e.g., the meaning expressed by ``he's not [un]reliable \emph{but he is kind of flaky}'', with prosodic focus on [un-]), potentially the result of politeness \cite{Yoon2017}.
This flexibility in interpretation is potentially supporting by our text-based experiment, which leaves open the question of how exactly our stimuli would sound to the ear; prosodic focus can more strongly constrain this interpretation, which will be important to clarify using speech-based experiments. 
...
Considerable empirical work on negation has focused on \emph{negative strengthening} \cite{Ruytenbeek2017, Gotzner2018}, and indeed we find convergent evidence in Experiment~2 (single utterances) that negated positives and morphological antonyms are interpreted similarly. H
owever, one might expect that the presence of the morphological antonym in the same context (i.e., our \emph{multiple utterances} condition of Experiment~2) would weaken the interpretation of the negated positive, but in fact, we observe the opposite in our data: When presented with multiple utterances, it is the morphological antonym that tends to be strengthened; the interpretation of the negated positive adjective remains unchanged, suggesting that in the absence of further context, morphological antonyms like \emph{unhappy} are interpreted more like simple contradictions.
Our results are also consistent with a recent electrophysiological processing study which found that negated antonyms (e.g., \emph{not impossible}) were processed more easily than positive adjectives (e.g., \emph{possible}) when participants had prior expectations that the degree was low \cite<e.g., something was very likely to difficulty;>{schiller2017solving}.
