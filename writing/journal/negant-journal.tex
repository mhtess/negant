...
\abstract{
Language provides multiple ways of conveying the opposite: A person \emph{not happy} can be \emph{unhappy}, \emph{sad}, or perhaps neither, just \emph{not happy}. Rather than being redundant, we hypothesize that uncertainty about the meaning of negation markers allows listeners to derive fine-grained distinctions among these various alternatives. We formalize this hypothesis in a probabilistic model of gradable adjectives (e.g., \emph{happy}), and use this to address an outstanding puzzle: how to interpret negated antonyms (e.g., \emph{not unhappy}). Our model makes surprising additional predictions about a putative difference between morphological antonyms (\emph{unhappy}) and negated positives (\emph{not happy}): Listeners should judge \emph{unhappy} as more sad than \emph{not happy} only when confronted with alternatives in context. Finally, if listeners actively maintain uncertainty about the meanings of negation markers, they should derive non-redundant meanings from flagrantly redundant double negations (e.g., \emph{not not happy}). Three behavioral experiments confirm consistent orderings of interpretations that interact with the presentational context in the way predicted. These findings support the hypothesis that listeners represent uncertainty even about the most logical elements of language.}
...
In other words, \enquote{not unhappy} (a \emph{negated antonym}) could indicate a slightly positive state, below that of \enquote{happy}  but above being neutral. %, where feelings that are neither positive nor negative reside. 
...
Without further assumptions, however, this analysis predicts an interpretation of \enquote{not unhappy} as indicating neutral feelings (i.e., \emph{neither unhappy nor happy}), contra \citeA{Jespersen1924}'s intuition that \enquote{not unhappy} indicates a slightly positive state.
Further, the logic of communicative reasoning changes should a speaker consider producing \enquote{not happy} \cite{Blutner2004:pragmatics}, though little agreement again is to be found on what \enquote{not happy} means (Figure~\ref{fig:happy-scale}B): \citeA{Jespersen1917:Negation} and \citeA{Blutner2004:pragmatics} believe that \enquote{not happy} means the same as \enquote{unhappy}, while \citeA{Krifka2007:Negated-antonyms} disagrees, citing examples like:
...
\emph{Contrary} opposites, on the other hand, can both be false (e.g., \emph{neither tall nor short}). %\emph{tall}~and~\emph{short}). 
In addition, contradictory opposition can be iterated (a number that is not not even is even) while contrary opposition cannot.\footnote{
...
We see the disagreement among linguists as to whether \enquote{unhappy} and \enquote{not happy} convey the same meaning as stemming from uncertainty about the how the contradictory~vs.~contrary distinction manifests in natural language markers (\enquote{not}, \enquote{un-}).
Given this ambiguity, a listener who hears only a single adjective phrase in isolation (e.g., \enquote{Jones is unhappy}) has no basis from which to decide whether contrary or contradiction is intended. 
Such uncertainty leads the model to impart no meaning difference between \emph{not happy} and \emph{unhappy}.
At the same time, a listener who hears double negations like \emph{not unhappy} can use the fact that a speaker has intentionally produced two negatives as evidence that the speaker intends to convey a contradiction about a contrary. 
The only logically possible alternative is that the speaker intended two contradictions, but if this were so, the speaker could have just used the positive adjective (\emph{happy}) because it is simpler and has same meaning as a double contradiction.
This mutual-exclusivity kind of reasoning is also predicted to occur when a speaker uses two distinct negations in the same context (e.g., \enquote{Jones is not happy, while Smith is unhappy}, also Krifka's example above); that is, the model predicts that \emph{unhappy} is intended to convey more negative feelings than \emph{not happy} when the model observes the speaker using both kinds of negation in the same context.
...
We construct a computational model for interpreting double negations by drawing on the tools of formal semantics and probabilistic models of pragmatics \cite{Franke2015a, Goodman2016:RSA}. 
...
Eqs.~\ref{eq:L1}--\ref{eq:L0} describe a recursive Bayesian model in the Rational Speech Act tradition \cite{Franke2015a, Goodman2016:RSA}, in which the listener  has additional uncertainty about how their interlocutor (speaker $S_1$) uses negation markers to convey contradictory~vs.~contrary negations \cite{Bergen2016}.
...
The pragmatic listener (Eq.~\ref{eq:L1}) has interprets an utterance by reasoning about three variables: the intended meaning or degree $x$, the threshold beyond which a scalar adjective is literally true  $\theta$, and the speaker's lexicon \(\mathcal{L}\) describing how the speaker uses negation. 
...
First, we compare to a vanilla RSA model which has neither uncertainty about negation nor vagueness in the meaning of the adjective \cite{Frank2012}.\footnote{
...
Finally, we construct a \emph{bonafide contraries} model, building on \citeA{Lassiter2015}'s vague RSA model by assuming morphological antonyms (\emph{un-}) convey contrary negation (e.g., \emph{unhappy} is to \emph{happy} how \emph{short} is to \emph{tall}).
...
Contraries and Contradictions & Yes       & Yes                         & $x > \theta_1$ & $x  < \theta_2$                  & $x < \theta_1$                   \\% & Contraries and Contradictions \\
...
When the model hears multiple utterances in the same context, the model does indeed predict a meaning difference between \enquote{unhappy} and \enquote{not happy}: \enquote{unhappy} is more sad than \enquote{not happy}, producing the ordering hypothesized by \citeA{Krifka2007:Negated-antonyms}.
...
\caption{Model predictions for interpretations of antonym pairs and their negations. Model predictions use the following minimally assumptive model parameters: $P(x) = \text{Uniform}(0, 1); \alpha = 1; \text{cost}(\mathit{un}) = \text{cost}(\mathit{not}) = 3$}.\label{fig:modelPredictions}
\end{figure}
...
\caption{\red{fix typo ``four friend of theirs''} Example experimental trials for (A) single utterance (Expts. 1, 2) and (B) multiple utterances conditions (Expt. 2). ``in the world'' wording for endpoints was used in Expt. 2. (A) shows a trial from a morphological antonym set while (B) shows a lexical antonym set.}\label{fig:experiment-slides}
\end{figure}
...
These numbers follow from the intention of getting approximately 45 ratings for each unique adjective in the experiment.
The experiment took average 5 minutes and participants were compensated \red{\$0.80.}
...
The only difference in this experiment is that participants are presented with the following alternatives: positives, negated positives, negated morphological anotnyms, and negated negated positives (e.g., \emph{happy}, \emph{not happy}, \emph{not unhappy}, \emph{not not happy}).
...
In this paper, we aimed to explain the modal interpretations of antonym pairs and their negations, but both intuition and our empirical data suggest more nuance and variability in participants' intuitions. 
