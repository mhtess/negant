...
It is classically explained via alternative-based pragmatic reasoning: If a speaker says they are ``not good'' when they could also have said they are ``fine'', the implication is that they are rather bad \cite{Horn1989:Natural}. 
...
This flexibility in interpretation is potentially supported by our text-based experiment, which leaves open the question of how exactly our stimuli would sound to the ear; prosodic focus can more strongly constrain this interpretation, which will be important to clarify using speech-based experiments. 
...
Considerable empirical work on negation has focused on \emph{negative strengthening} \cite{Ruytenbeek2017, Gotzner2018}, and indeed we find convergent evidence in Experiment~2 (single utterances) that negated positives and morphological antonyms are interpreted similarly. However, one might expect that the presence of the morphological antonym in the same context (i.e., our \emph{multiple utterances} condition of Experiment~2) would weaken the interpretation of the negated positive, but in fact, we observe the opposite in our data: When presented with multiple utterances, it is the morphological antonym that tends to be strengthened; the interpretation of the negated positive adjective remains unchanged, suggesting that in the absence of further context, morphological antonyms like \emph{unhappy} are interpreted more like simple contradictions.
