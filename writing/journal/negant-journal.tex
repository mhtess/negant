...
More broadly, pinning down the meaning of negation---one of the most logical elements of language---is crucial for so much of psychological science that depends upon using linguistic information to convey task instructions and task content.
...
Under this explanation, however, one might expect that the presence of the morphological antonym in the same context (i.e., our \emph{multiple utterances} condition of Expt.~2) would weaken the interpretation of the negated positive, but in fact, we observe the opposite in our data: When presented with multiple utterances, it is the morphological antonym that tends to be strengthened; the interpretation of the negated positive adjective remains unchanged, suggesting that in the absence of further context, morphological antonyms like \emph{unhappy} are interpreted more like simple contradictions.
Furthermore, this pragmatic, negative strengthening explanation has never been articulated formally and so it  is not clear what pragmatic components are necessary to derive this inference. 
This account is often presented via alternative-based pragmatic reasoning: A speaker could have they are ``fine'' and the fact that they chose to say they not ``not good'' implies that they are rather bad \cite{Horn1989:Natural}. 
...
This flexibility in interpretation is potentially supporting by our text-based experiment; prosodic focus can more strongly guide this interpretation, which will be important to clarify using speech-based experiments. 
...
Our results are consistent with a recent electrophysiological processing study which found that negated antonyms (e.g., \emph{not impossible}) were processed more easily than positive adjectives (e.g., \emph{possible}) when participants had prior expectations that the degree was low \cite<e.g., something was very likely to difficulty;>{schiller2017solving}.
...
Our findings and modeling suggest that even seemingly rigid linguistic elements like negation may have some kind of ambiguity or nuance to their meaning.
...
Intentions need not be nefarious, however, to lead to confusion: Listeners may reason about the ambiguity of negation even when speakers do not intend.
