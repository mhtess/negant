...
In other words, \enquote{not unhappy} (a \emph{negated antonym}) could indicate a slightly positive state, below that of \enquote{happy}  but above a \emph{zone of indifference} (Sapir, 1944). %, where feelings that are neither positive nor negative reside. 
...
Without further assumptions, however, this analysis predicts an interpretation of \enquote{not unhappy} in the zone of indifference (i.e., \emph{neither unhappy nor happy}), contra \citeA{Jespersen1924}'s intuition that \enquote{not unhappy} is a slightly positive state.
...
\emph{Contrary} opposites, on the other hand, can both be false (e.g., \emph{tall}~and~\emph{short}). 
We suggest that the disagreement as to whether or not the same meaning is conveyed by \enquote{unhappy} and \enquote{not happy} stems from uncertainty about the how the various kinds of semantic opposition map onto the natural language negation markers  (\enquote{not}, \enquote{un-}).
Furthermore, we argue that interpreting negated antonyms like \enquote{not unhappy} not only involves uncertainty about negation but how that uncertainty interacts the vagueness of predicates like \emph{happy} or \emph{tall}.
...
If a negation marker (e.g., \enquote{not}) creates a contradictory opposite, then the resulting meaning would simply be that the degree is less than or equal to the same threshold: \(\neg \mbox{ $[\![ happy ]\!]$}(x, \theta_1): x \leq \theta_1\).
...
The act of producing a negated antonym (\enquote{not unhappy}) could then act as a signal towards the kinds of oppositions that the speaker had in mind (e.g., that the speaker intends to convey a contradictory negation of a contrary opposite).
...
Eqs.~\ref{eq:L1}--\ref{eq:L0} describe a recursive Bayesian model in the Rational Speech Act tradition \cite{Franke2015a, Goodman2016:RSA}, wherein a pragmatic listener $L_1$ resolves the meaning of an utterance $u$ by reasoning about why a rational speaker $S_1$ would have bothered to produce said utterance.
