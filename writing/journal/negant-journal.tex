...
The intuitions are subtle, and not universally agreed upon (see Orwell's 1946 quote above). 
The interpretative landscape is further complicated by the fact that the meanings of \emph{not happy} and \emph{unhappy} are not obviously different from one another.
...
How should the negation markers \emph{not} and \emph{un-} map onto the different kinds of negation?
A standard Aristotelean hypothesis about the meaning of explicit natural language negation markers (\emph{not}, \emph{un-}) assigns one to be a contradictory opposite and another to be a contrary (Figure \ref{fig:opp-square}B); a double negative (\emph{not unhappy}) in this analysis would convey a contradiction of a contrary, resulting in a meaning distinct from that of the positive adjective (\emph{happy}), though also enforcing the contrary (\emph{unhappy}) has a more negative meaning than the contradiction (\emph{not happy}; Figure \ref{fig:opp-square}C; \citeNP{Horn1991:Duplex, Krifka2007:Negated-antonyms}).
...
Due to logical and pragmatic considerations, listeners can infer that a speaker who produces a double-negation meant to do so in a non-redundant manner (i.e., \emph{not unhappy} means something different from \emph{happy}). 
...
The basic architecture of such a model is that a listener interprets an utterance like \emph{not unhappy} as being intentionally produced by a speaker trying to describe their feelings or the state of the world to the listener  \cite<Rational Speech Act models; >{Franke2015a, Goodman2016:RSA}.
...
We additionally discovered and confirmed a surprising empirical result: \emph{unhappy} and \emph{not happy} are not differentiated in meaning when presented in isolation, but are differentiated when presented in the same context.
...
We acknowledge that the usage of the same negation marker twice (e.g., \emph{not not}) is uncommon, though examples are easy to find on webpages and news articles.
...
This result suggests a radical view of uncertainty about negation markers, where the uncertainty is not only resolved at the level of individual speakers (e.g., speaker A uses \emph{un-} to mean contrary and \emph{not} as a contradiction) but that it can be computed for each individual token-usage of a word (e.g., \emph{not} can mean both a contrary and contradiction for a given speaker).
...
Contraries act as lexicalized form of negation because their meanings refer to semantic variables that are distinct from the positive adjective (in particular, a different threshold; see SI), whereas contradictions refer to the same semantic variables. 
...
Finally, though our experiments are conducted in English, we expect these patterns to hold cross-linguistically, at least for languages that do not normally allow for negative concord.
...
Our results suggest that if a speaker claims an individual is ``not not innocent'', the speaker can simultaneously convey that the person is innocent as well as that they are at least somewhat guilty.
