...
Consequently, when listeners observe an utterance of a double negation like \emph{not unhappy}, they  look for ways of rationalizing why the speaker has not used the simpler expression \emph{happy}, if that were the meaning that she wants to communicate.
If even logical vocabulary, like negation markers, is prone to flexible interpretation, then what are its limits?
...
In this paper, we propose that the logic of negated morphological antonyms (\emph{not unhappy}) proceeds via a listener reasoning about why a speaker used two distinct negation markers (\emph{not} \& \emph{un-}) and concluding that the speaker must have had different meanings in mind (contrary \& contradiction) for the different negation markers.
Yet, one can grammatically produce a double negative flagrantly using the same negation marker twice (e.g., \emph{not not happy}). 
Is the inferential mechanism in the human mind that produces meanings for negated antonyms like \emph{not unhappy} sufficiently flexible to accommodate double negations that rely upon the iterative application of the same logical word (\emph{not not})?
If so, that would suggest that communicative reasoning can overturn one of the basic logical structures in language.
