...
These findings argue against simplistic assumptions of language meanings---even logical elements in language---often used throughout psychological science.
}
...
One explanation for this result is via a process known as \emph{negative strengthening}, where the negated positive adjectives (e.g., \emph{not intelligent}) receive a stronger interpretation than literally implied \cite<e.g.,  rather stupid;>{Horn1989:Natural, Ruytenbeek2017, Gotzner2018}. 
If this were so, one might expect that the presence of the morphological antonym in the same context (i.e., our \emph{multiple utterances} condition of Expt.~2) would weaken the interpretation of the negated positive.
In fact, we observe the opposite in our data: When presented with multiple utterances, it is the morphological antonym that tends to be strengthened; the interpretation of the negated positive adjective remains unchanged. 
Furthermore, it is not clear how pragmatically this negative strengthening explanation should account for the pattern of data we observe.
...
Including ``fine'' into alternative set of a comparable RSA model to the ones we present does not capture the key behavioral patterns we observe (see SI). 
...
We provided a computational solution to an outstanding puzzle in natural language understanding: How to interpret double negatives \cite<e.g., \emph{not unhappy}; >{Horn1991:Duplex, Krifka2007:Negated-antonyms, Rett2014:eval}.
...
\mht{verify this modeling result. why is ``not unhappy'' so strong if it is likely a response to a polar QUD? one possibility: with polar QUD, you get interpretation by reasoning about only 2 utterances... no competition with ``happy''...}
...
Our findings and modeling suggest that even seemingly rigid logical elements like negation may have some kind of ambiguity or meaning nuances. For one, this can be exploited by speakers in very subtle ways giving rise to phenomena as dog whistles. For another, this could lead to confusion in situations where speakers are not trying to exploit ambiguity but listener's reason about it. 
