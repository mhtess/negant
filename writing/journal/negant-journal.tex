...
In natural language, however, it is not always clear what should happen:
...
The most straightforward analysis is that \enquote{not unhappy}  logically overlaps in meaning with \enquote{happy} (Figure~\ref{fig:happy-scale}A) and thus a speaker who chooses to say \enquote{not unhappy} must have meant \emph{not unhappy but not happy} via pragmatic implicature \cite{Horn1991:Duplex}. 
Without further assumptions, however, this predicts an interpretation of \enquote{not unhappy} in the zone of indifference, contra \citeA{Jespersen1924}'s intuition that \enquote{not unhappy} is a slightly positive state.
Further, the logic changes should a speaker consider producing \enquote{not happy} (Blutner, 2004),  though again, little agreement is to be found on what \enquote{not happy} means (Figure~\ref{fig:happy-scale}B): \citeA{Jespersen1917:Negation} and \citeA{Blutner2004:pragmatics} believe that \enquote{not happy} means the same as \enquote{unhappy}, while \citeA{Krifka2007:Negated-antonyms} disagrees, citing examples like:
...
A contrary opposition is one where both predicates cannot be true at the same time, but both can be false (e.g., \emph{tall}~and~\emph{short}). 
A contradictory opposition is one where the falsity of one predicate entails the truth of the other (e.g., even~vs.~odd positive integers). 
We hypothesize that listeners have uncertainty about whether negation markers  (\enquote{not}, \enquote{un-}) express contradictory or contrary opposition and that this uncertainty interacts with the vagueness of gradable adjectives like \enquote{happy} to produce logically subtle meanings in context.
