...
The most radical view of uncertainty in negation additionally predicts that an utterance involving the same negation marker twice (e.g., \emph{not not happy}) should too mean something different than just \emph{happy}.
...
If Mary is ``not unhappy'', does that mean she is happy?
...
\end{quote}
\noindent In other words, \enquote{not unhappy} (a \emph{negated antonym}) should indicate a slightly positive state, below that of \enquote{happy}  but perhaps more positive than neutral (i.e., above the \emph{zone of indifference}; \citeNP{sapir1944grading}). %, where feelings that are neither positive nor negative reside. 
The intuitions are subtle, and not universally agreed upon (see \citeNP{orwell1946politics}'s quote above). 
The interpretative landscape is further complicated by the fact that the meanings of \enquote{not happy} and \enquote{unhappy} are not obviously different from one another.
Some \cite<e.g., >{Jespersen1917:Negation, Blutner2004:pragmatics} argue that the two expressions have the same meaning, while others \cite<e.g., >{Krifka2007:Negated-antonyms} disagree, citing examples like:
...
The idea that two negatives (``not'' + ``un-'') don't exactly cancel, while also plausibly meaning the same thing, is troubling from both a logical perspective \cite{Horn1989:Natural, Krifka2007:Negated-antonyms}  as well as a legal one, where double negations are surprisingly common \cite{tiersma1999legal}.
...
A standard Aristotelean hypothesis about the meaning of explicit natural language negation markers (\enquote{not}, \enquote{un-}) assigns one to be a contradictory opposite and another to be a contrary (Figure \ref{fig:opp-square}B); a double negative (``not unhappy'') in this analysis would convey a contradiction of a contrary, resulting in a meaning distinct from that of ``happy'', though also stipulating that ``unhappy'' is more negative than ``not happy'' (Figure \ref{fig:opp-square}C; \citeNP{Horn1991:Duplex, Krifka2007:Negated-antonyms}).
An Orwellian analysis, on the other hand, only acknowledges contradictory opposition as an interpretation for negation markers; ``unhappy'' and ``not happy'' have the same interpretation, and doubles-negatives are posited to be entirely redundant \cite<Figure \ref{fig:happy-scale}; >{orwell1946politics}.\footnote{Though we refer to these models as \emph{Aristotle} and \emph{George Orwell}, they are derived from different mappings of English negation markers onto logical operations. Aristotle did not speak English, and thus would personally have not had much to say about ``not'' or ``un-'' as we understand them. George Orwell was an English speaker, though not a formal linguist or scientist. The important feature of these individuals was that their ideas have natural analogues to the semantic question we ask here.}


We propose a third, hybrid alternative: the logical distinction between contradictory~vs.~contrary opposition creates an ambiguity in the meanings of explicit natural language negation markers (\enquote{not}, \enquote{un-}). % and that  listeners maintain uncertainty about the mapping between logical negations and natural language negation% (\enquote{not}, \enquote{un-}) in a stable, context-invariant manner. 
This uncertain negation hypothesis predicts that listeners derive a unique interpretation for double negatives, but not because the negation markers have \emph{a priori} different meanings (as the Aristotolean analysis supposes), but because people entertain a hypothesis space of logical possibilities when interpreting negation markers (Figure \ref{fig:meanings}). 
Due to logical and pragmatic considerations, listeners can infer that a speaker who produces a double-negation meant to do so in a non-redundant manner (i.e., ``not unhappy'' means something different from ``happy''). 
But unlike the Aristotelean hypothesis, this alternative hypothesis further predicts that \emph{not happy} and \emph{unhappy} inherit the same set of possible meanings, and thus two expressions are interpreted in the same way (qualitatively similar to the Orwellian analysis for \emph{not happy}~vs.~\emph{unhappy}).
...
Our hypotheses about the meanings of negation markers are semantic hypotheses about the meaning of these linguistic expressions.  
Linguistic meanings, like mental representations, are not directly observable; thus, we formalize and test these hypotheses in probabilistic models of rational communication which provide the necessary linking function between these semantic hypotheses and observable behavior. 
The basic architecture of such a model is that a listener interprets an utterance like ``not unhappy'' as being intentionally produced by a speaker trying to describe their feelings or the state of the world to the listener  \cite<Rational Speech Act models; >{Franke2015a, Goodman2016:RSA}.
...
Hearing multiple utterances by the same speaker, however, can provide the listener more information about how the speaker uses language, as in \citeA{Krifka2007:Negated-antonyms}'s example above (``I wasn't unhappy, just not happy''). 
...
Crucially, this inference results in the \emph{Uncertain Opposites} model predicting a meaning difference between \enquote{unhappy} and \enquote{not happy}: \enquote{unhappy} is more negative than \enquote{not happy}, producing the ordering hypothesized by \citeA{Krifka2007:Negated-antonyms} when both are used in the same context.
...
Our novel \emph{uncertain negation} hypothesis is not just an account of a few particular linguistic puzzles, but is a case study in how language users manipulate and contort language to navigate and communicate the gradations of their conceptual landscape. 
...
We design experiments to test the four behavioral predictions that result from our uncertain negation model (Figure \ref{fig:modelPredictions}): (1.) Negated antonyms (``not unhappy'') receive different interpretations than simple positives (``happy''); (2.) Negated antonyms tend to receive a slightly positive interpretation (\emph{a la} \citeA{Jespersen1924}'s intuition above); (3.) Negated positives and morphological antonyms (``not happy'' and ``unhappy'') receive the same interpretation when presented in isolation; (4.) Negated positives and morphological antonyms receive different interpretations when presented in the same context. 
%We test these predictions across three experiments using antonyms constructed by morphology (e.g., ``un-'' + adjective).
We expect these predictions to apply to words with explicit negation markers (e.g., antonyms constructed by morphology such as ``un-'' +``happy''); as a control condition, we examine antonyms which do not have overt negation (e.g., \emph{tall} vs. \emph{short}).
...
Finally, Experiment 3 interrogates how specific the patterns of inferences are to morphological negation (\emph{un-}) as opposed to negation more broadly by testing double negatives that use the same negation marker  (\emph{flagrant double negatives} e.g., ``not not happy'').
...
All of our adjectives were context-dependent, relative adjectives consistent with the definitions of \cite{Kennedy2007} and \cite{kennedy2005scale}.
...
To taste these, we built a mixed-effects regression model with random by-participant and by-item intercepts and slopes (where the slopes refer to the effect of the adjective type---happy vs. not happy vs. not not happy vs. unhappy). \footnote{We do this using a forward-difference coding scheme in order to test the relevant contrasts---happy vs. not not happy; not happy vs. unhappy---within the same model. }
As is visually apparent, the two negatives did not make a positive: ``not not happy'' received a markedly different interpretation than simply ``happy'' (\(\beta = \rlgetnum{expt3_glmer_adjective.csv}{Rowname}{adjective_type_rescaled3}{Estimate}{3}\), t\((\rlgetnum{expt3_glmer_adjective.csv}{Rowname}{adjective_type_rescaled3}{df}{0}) = \rlgetnum{expt3_glmer_adjective.csv}{Rowname}{adjective_type_rescaled3}{t.value}{2}, p < 0.001\)).
In addition,  the result from Experiment 2 (multiple utterances condition) replicated for the morphological antonyms: ``not happy'' and ``unhappy'' were differentiated in meaning (\(\beta = \rlgetnum{expt3_glmer_adjective.csv}{Rowname}{adjective_type_rescaled1}{Estimate}{3}\), t\((\rlgetnum{expt3_glmer_adjective.csv}{Rowname}{adjective_type_rescaled1}{df}{0}) = \rlgetnum{expt3_glmer_adjective.csv}{Rowname}{adjective_type_rescaled1}{t.value}{2},  p =  \rlgetnum{expt3_glmer_adjective.csv}{Rowname}{adjective_type_rescaled1}{Pr...t..}{3}\)).
Contrary to our hypothesis, however, we did not find evidence that the flagrant double negative ``not not happy'' received on-average a positive interpretation (\(\beta = \rlgetnum{expt3_notnot_vs_baseline.csv}{Rowname}{(Intercept)}{Estimate}{3}\), t\((\rlgetnum{expt3_notnot_vs_baseline.csv}{Rowname}{(Intercept)}{df}{0}) = \rlgetnum{expt3_notnot_vs_baseline.csv}{Rowname}{(Intercept)}{t.value}{2},  p =  \rlgetnum{expt3_notnot_vs_baseline.csv}{Rowname}{(Intercept)}{Pr...t..}{2}\)); inspection of the random-effects suggested this was due to large subject-wise variation in the interpretation of ``not not happy''. 
We discuss this observation further in the General Discussion. 


\begin{figure*}[h]
\centering \includegraphics[width=0.6\linewidth]{figs/expt3_ridges_wCIs} 
...
We additionally discovered and confirmed a surprising empirical result: \emph{unhappy} and \emph{not happy} are not immediately differentiated, except when both are present in the same context.
...
This work thus carries with it an account of a robust linguistic intuition: Potentially equivalent utterances receive differential interpretations when produced by the same speaker in close proximity. %\mht{are the other lexical uncertainty papers not an instance of this general point as well?}
...
In our final experiment, we showed that flagrant double negative statements such as \emph{not not happy} receive an interpretation similar to that of more conventional negated antonyms (\emph{not unhappy}) as indicating a slightly positive state. 
...
This result suggests a radical view of uncertainty about negation markers, where the uncertainty is not only resolved at the level of individual speakers but that it can be computed for each individual token-usage of a word.
...
We've extended earlier work showing that negated lexical antonyms (e.g., ``not dull'') are distinct from positive adjectives (e.g., ``sharp'') when presented simultaneously \cite{giora2005negation} (Expt.~4) to both the case of morphological antonyms (e.g., ``not unhappy'') and isolated presentation (single utterance conditions). 
%ound that participants rate . 
We also provide convergent evidence that supports the phenomenon of \emph{negative strengthening} or \emph{inference towards the antonym}, where negating an adjective (e.g., ``not intelligent'') can lead to the stronger interpretation than literally implied \cite<e.g., ``rather stupid''; >{Ruytenbeek2017, Gotzner2018}. 
Not only do our empirical data further elucidate this phenomena by our straight-forward response measurements and comparisons to other adjective forms in different presentational contexts, \red{our modeling approach provides an explanation for negative strengthening through competition with alternative utterances. } \mht{<-- include or change?}
...
The two types of negation meaning we considered---contrary and contradictory opposition---can be seen as a \emph{lexicalized} form of opposition and a \emph{compositional} rule, respectively.
...
The fact that the meaning of explicit negation markers like \emph{not} and \emph{un-} is uncertain introduces an ambiguity that can be exploited by speakers.
If a speaker claims an individual is ``not not guilty'', the speaker can simultaneously convey that the person is innocent as well as that they are at least somewhat (but perhaps not completely) guilty.
