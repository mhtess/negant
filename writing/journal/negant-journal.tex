...
As suggested by Experiment 3a in which the two double negatives are not presented in the same context \mht{either add numbers for E3a or drop}, the doubly-negated positive adjective receives a lower overall rating than the negated morphological antonym (\brmresultsp{expt3b_brm_contrasts_10k.csv}{morph_negnegpos_v_negant}; \effsize{expt3b_brm_effsize_10k.csv}{morph_negnegpos_v_negant}).
...
One explanation for this result is via a process known as \emph{negative strengthening}, wherein a negated positive adjective receives a stronger interpretation than literally implied \cite<e.g., \emph{not intelligent} $\rightarrow$ rather stupid;>{Horn1989:Natural, Ruytenbeek2017, Gotzner2018}. 
...
The simple assumption that negation particles (``not'') are more likely under polar QUDs (\emph{is John happy?}) leads to ``not happy'' and ``unhappy'' being interpreted similarly, but also washes out the difference between ``not unhappy'' and  ``happy''? (see SI).
QUDs are likely play a role in understanding negation and the modeling approach we present here provides a framework for elaborating more complex hypothesis about the relationship between the QUD and utterances involving negation.
...
The interpretations we observe empirically for negated antonyms  indicate a slightly positive state on average, but with consistently slightly negative interpretations as well (e.g., the meaning expressed by ``he's not [un]reliable \emph{but he is kind of flaky}'', with prosodic focus on [un-]), potentially the result of politeness \cite{Yoon2017}.
...
\cite{Ruytenbeek2017, Gotzner2018}, and indeed we find convergent evidence that negated positives and morphological antonyms are interpreted similarly (Experiment 1, Experiment~2 single utterances). However, one might expect that the presence of the morphological antonym in the same context (i.e., our \emph{multiple utterances} condition of Experiment~2) would weaken the interpretation of the negated positive (e.g., ``not intelligent'' \emph{but not stupid}), but in fact, we observe the opposite: When presented with multiple utterances, it is the morphological antonym that tends to be strengthened, suggesting that in the absence of further context, morphological antonyms like \emph{unhappy} are interpreted more like simple contradictions.
...
Formalizing the structure of this ambiguity and how listeners reason it is an important step to understanding the complexities in our language. 
