...
\title{Not stating the opposite: Chaining negations creates nonredundant meanings}
...
\abstract{
Language provides multiple ways of conveying the opposite: A person \emph{not happy} can be \emph{unhappy}, \emph{sad}, or perhaps neither, just \emph{not happy}. It has long been attested that, contra standard logic, chaining negation markers creates nonredundant meanings: A person \emph{not unhappy} is not exactly \emph{happy}.
...
We formalize this hypothesis in a computational model of language understanding and show that chaining negations is expected to result in nonredundant meanings only for propositions that convey continuously-valued dimensions and not for binary distinctions.
This hypothesis also makes surprising additional predictions about single negations (\emph{unhappy} or \emph{not happy}): The two have identical meanings when presented in isolation, but \emph{unhappy} is more negative than \emph{not happy} when a speaker uses both in the same context.  
The most radical prediction derived from this account is that flagrant double negations that use the same negation marker (e.g., \emph{not not happy}) should too have nonredundant meanings. 
...
When Obama said that the time to negotiate was \emph{not unlimited}, did he mean that time was limited? 
...
Such a subtle, nonredundant behavior of chaining negations (\emph{not} + \emph{un-}) is troubling from a formal perspective. 
...
%Understanding the behavior of negation onto each other is important for interpreting the law, describing emotions, and navigating everyday conversations. 
...
According to this analysis, however, \enquote{not unhappy} indicates neutral feelings: \emph{neither happy nor unhappy}, contra the intuition expressed by \citeA{Jespersen1924} that \enquote{not unhappy} indicates a slightly positive state.
It is plausible that listeners reason entertain other possibilities regarding what the speaker could have said: The speaker could have  said single negations (\enquote{not happy} or \enquote{unhappy}; \cite{Blutner2004:pragmatics}).
Introducing new alternatives only raises more questions: Are \enquote{unhappy} and \enquote{not happy} different in meaning?
