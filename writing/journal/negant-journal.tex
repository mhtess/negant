...
\abstract{
Language provides multiple ways of conveying the opposite: A person \emph{not happy} can be \emph{unhappy}, \emph{sad}, or perhaps neither, just \emph{not happy}. It has long been attested that, contra standard logic, chaining negation markers creates nonredundent meanings: A person \emph{not unhappy} is not exactly \emph{happy}.
We hypothesize this behavior stems from listeners entertaining a multiplicity of opposite meanings, which when presented simultaneously (as in \emph{not unhappy}) allows for the creation of nonredundent meanings. 
...
We formalize this hypothesis in a computational model of language understanding and show that chaining negations is expected to result in nonredundent meanings only for propositions that convey continuously-valued dimensions and not for binary distinctions.
...
The most radical prediction derived from this account is that flagrant double negations that use the same negation marker (e.g., \emph{not not happy}) should too have nonredundent meanings. 
...
In other words, \enquote{not unhappy} (a \emph{negated antonym}) could indicate a slightly positive state, below that of \enquote{happy}  but perhaps not neutral. %, where feelings that are neither positive nor negative reside. 
