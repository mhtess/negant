...
But we can also use the tools already available to us, at the time we want to convey their message, to navigate and communicate the gradations of our thoughts and feelings (e.g., through metaphoric language; \citeNP{lakoff2008}).
...
The interpretative landscape is further complicated by the fact that the meanings of \enquote{not happy} and \enquote{unhappy} are not obviously different.
...
Our hypotheses about the meanings of negation markers are semantic hypotheses about the linguistic expressions mean. 
...
The basic architecture of such a model is that a listener interprets an utterance like ``not unhappy'' as being intentionally produced by a speaker trying to describe their feelings or the state of the world to them  \cite<Rational Speech Act models; >{Franke2015a, Goodman2016:RSA}.
The mathematical details are not important here, beyond noting that the models build on top of existing formalized theories of probabilistic language understanding, the semantics and pragmatics of gradable adjectives \cite{Kennedy2007, Lassiter2015}, and ambiguity in the meanings of words \cite{Bergen2016}. 
...
We design experiments to test three straight-forward behavioral predictions that result from the \emph{Uncertain Opposites} model (Figure \ref{fig:modelPredictions}, \emph{single utterance}): (1) negated antonyms (``not unhappy'') receive different interpretations than simple positives (``happy''); (2) negated antonyms tend to receive a slightly positive interpretation (\emph{a la} \citeA{Jespersen1924}'s intuition above); and (3) negated positives and morphological antonyms (``not happy'' and ``unhappy'') receive the same interpretation. 
...
%Across three experiments, we find support for our \emph{uncertain opposites} hypothesis.
%Such an account indicates a new avenue for exploitation by speakers: The meanings of seemingly logical words that explicitly convey negation carry a variety of interpretations.
