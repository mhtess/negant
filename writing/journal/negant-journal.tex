...
The interpretation of double negatives is interesting from both a logical perspective \cite{Horn1989:Natural, Krifka2007:Negated-antonyms} as well as a legal one, where double negations are surprisingly common \cite{tiersma1999legal}.
...
The models spelled out in the SI conservatively combine previous ideas on the interpretation of gradable adjectives \cite{Kennedy2007, Lassiter2015} and ambiguity in the meanings of words \cite{Bergen2016}. 
...
The general idea in the background is that listeners rationalize utterances of speakers, especially when the utterances are unexpected \cite{Grice1975}.
Consequently, when listeners observe an utterance of a double negation like \emph{not unhappy}, they look for ways of rationalizing why the speaker has not used the simpler expression \emph{happy}, if that were the meaning that she wanted to communicate.
The most extreme form of this hypothesis is that the actual negation markers (\emph{not} \& \emph{un-}) are irrelevant to the conversational reasoning and interpretation; all that need occur is that a speaker use two negation markers. 
Thus, we also investigate  the dependence on linguistic form by asking whether participants interpret double negatives that flagrantly use the same negation marker twice (e.g., \emph{not not happy}) in a way analogous to how they interpret negated antonyms that use distinct negation markers (e.g., \emph{not unhappy}). 
Double negatives that flagrantly use the same negation marker twice are a strong test of the \ourmodel hypothesis, because in order to make sense of the utterance in a principled way, listeners would have to ascribe different meanings to the two different instances of \emph{not} (i.e., the utterance would indicate a contradiction of a contrary). 
...
\begin{figure}[t]
\centering\includegraphics{figs/happy-scale4}
...
20 adjective sets were constructed, 10 for lexical antonyms (\emph{short}) and 10 for morphological antonyms (\emph{unhappy}). 
...
As we did in \text{Expt.$\thinspace$1}, we evaluated our hypothesis that, in the \emph{single utterance condition}, morphological antonyms behave like the \emph{\ourmodel} model (i.e., show a partial ordering) while lexical antonyms show a true ordering (like \emph{bonafide contraries}).
...
\), t\(( \rlgetnum{expt2_glmer_singleUtt.csv}{Rowname}{adj_type1}{df}{0}) =  \rlgetnum{expt2_glmer_singleUtt.csv}{Rowname}{adj_type1}{t.value}{2}, p =  \rlgetnum{expt2_glmer_singleUtt.csv}{Rowname}{adj_type1}{Pr...t..}{3}\)).
...
\), t\(( \rlgetnum{expt2_glmer_singleUtt_lexBase.csv}{Rowname}{adj_type1}{df}{0}) =  \rlgetnum{expt2_glmer_singleUtt_lexBase.csv}{Rowname}{adj_type1}{t.value}{2}, p =  \rlgetnum{expt2_glmer_singleUtt_lexBase.csv}{Rowname}{adj_type1}{Pr...t..}{3}\)).
...
In this paper, we propose that the logic of negated morphological antonyms (\emph{not unhappy}) proceeds via a listener reasoning about why a speaker used two distinct negation markers (\emph{not} \& \emph{un-}) and concluding that the speaker must have had different meanings in mind (contradiction and contrary, respectively) for the different negation markers.
...
Is the inferential cognitive mechanism that produces meanings for negated antonyms like \emph{not unhappy} sufficiently flexible to accommodate double negations that rely upon the iterative application of the same logical word (\emph{not not})?
If so, then we would expect flagrant double negatives like \emph{not not happy} to be interpreted in a way similar to more canonical double negatives like \emph{not unhappy}. 
...
We built a mixed-effects regression model with random by-participant and by-item intercepts and slopes (where the slopes refer to the effect of the adjective type, forward-difference coded as before: happy vs.~not happy vs.~not not happy vs.~unhappy).
As is visually apparent, the two negatives did not simply cancel to make a positive: \emph{not not happy} received a significantly more negative interpretation than \emph{happy} (\(\beta = \rlgetnum{expt3_glmer_adjective.csv}{Rowname}{adjective_type_rescaled3}{Estimate}{3}\), t\((\rlgetnum{expt3_glmer_adjective.csv}{Rowname}{adjective_type_rescaled3}{df}{0}) = \rlgetnum{expt3_glmer_adjective.csv}{Rowname}{adjective_type_rescaled3}{t.value}{2}, p < 0.001\)).
This suggests that participants actively try to make sense of seemingly redundant, contradictory information in a way that would be informative for a speaker to produce. 
...
In Experiment 3, we discovered that this inference goes beyond using distinct expressions for negation: Double negative statements that use the same negation marker twice (e.g., \emph{not not happy}) are interpreted in a similar way to negated morphological antonyms (e.g., \emph{not unhappy}), indicating the underlying cognitive mechanism used to make sense of seemingly redundant, contradictory content (\emph{not not}) is guided by a principled underlying logic: Two negatives can be coherently combined if one is a contradiction and the other is a contrary.
This result also suggests a radical view of flexible communication with negation, wherein the same negation marker can be interpreted in two different ways within a single utterance.
