...
When subtle feelings arise that are more difficult to express in common speech, speakers may
resort to creative language \cite<e.g., metaphors;>{lakoff2008} or uncommon, perhaps novel terms like being ``plateaued'' \cite{bardwick1986plateauing} or residing in a ``zone of indifference'' \cite{sapir1944grading}.
...
Our models are of listeners who hear utterances (e.g., \emph{not unhappy}) and compute a posterior distribution over degrees of happiness, representing predictions about the interpretation of an utterance (Figure \ref{fig:modelPredictions}; see SI for mathematical details of each model), which we test in the experiments that follow.
...
In Experiment 3, we saw that this inference goes beyond using distinct negation markers: Expressions that use the same negation marker twice (e.g., \emph{not not happy}) are interpreted in a manner distinct from that of positive adjectives (e.g., \emph{happy}) and that of negated antonyms (e.g., \emph{not unhappy}). 
