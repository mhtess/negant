...
\title{Not stating the opposite: Double negations result in nonredundent meanings}
...
\abstract{
Language provides multiple ways of conveying the opposite: A person \emph{not happy} can be \emph{unhappy}, \emph{sad}, or perhaps neither, just \emph{not happy}. It has long been attested that, contra standard logic, two negatives create nonredundant meanings: A person \emph{not unhappy} is not exactly \emph{happy}.
We hypothesize this behavior stems from listeners entertaining a multiplicity of opposite meanings, which when presented simultaneously (as in \emph{not unhappy}) allows for the creation of nonredundant meanings. 
...
We formalize this hypothesis in a computational model of language understanding and show that double negations are theoretically expected to result in nonredundant meanings for propositions that convey continuously-valued dimensions and not for binary distinctions.
This hypothesis also makes surprising additional predictions about single negations conveyed in distinct ways (\emph{unhappy}~vs.~\emph{not happy}): The two are predicted to have identical meanings when presented in isolation, but \emph{unhappy} should be more negative than \emph{not happy} when a speaker uses both in the same context.  
The most radical prediction derived from this account is that blatant double negations which use the same negation marker (e.g., \emph{not not happy}) should too have nonredundant meanings. 
Three behavioral experiments confirm consistent orderings of interpretations that interact with the presentational context in the way predicted. 
...
Also, insofar as nonredundent meanings can be expressed through double negations, such language could provide new avenues for understanding the language of emotions, as in \emph{not unhappy} \cite{Lindquist2015, Satpute2016}. 
...
Introducing new alternatives only raises more questions, however: Are \enquote{unhappy} and \enquote{not happy} different in meaning?
