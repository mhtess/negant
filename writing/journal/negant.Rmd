---
title: "Not unreasonable: Uncertainty in the language of logic"
...
\begin{quote}
Banal statements are given an appearance of profundity by means of the ``not un-'' formation. [$\ldots$] It should be possible to laugh the ``not un-'' formation out of existence by memorizing this sentence: ``A not unblack dog was chasing a not unsmall rabbit across a not ungreen field.'' [@orwell1946politics, p.$\thinspace$357]
\end{quote}
...
In other words, "not unhappy"  (a *negated antonym*) could indicate a degree of happiness below that indicated by "happy" (a  *positive adjective*) but above those of "not happy" and "unhappy" [*negated positives* and *antonyms*, respectively; Fig.$\thinspace$\ref{fig:happy-scale}; @Krifka2007:Negated-antonyms].
...
Antonyms created by morphology (*morphological antonyms* e.g., "un"+"happy") could convey a *contrary* meaning, which is often conveyed by antonyms which have their own lexical entry (*lexical antonyms* e.g., "sad").
The key fact about contraries is that they can both be false (unlike logical *contradictions*): Jones may be neither tall nor short (contraries) but a natural number cannot neither be even nor odd [contradictions; @Horn1989:Natural].
Pragmatically, "not unhappy" should compete with "happy" as an alternative description, resulting in a contextually strengthened interpretation corresponding to a neutral or indifferent state [@Horn1991:Duplex], contra @Jespersen1924's intuition that "not unhappy" is a slightly positive state.
Such an analysis, however, further depends on the meaning of "not happy" [@Blutner2004:pragmatics], of which there is also little agreement: @Jespersen1917:Negation and @Blutner2004:pragmatics argue that "not happy" means the same as "unhappy", while @Krifka2007:Negated-antonyms disagrees, citing examples like:
...
<!-- The most straight-forward, compositional analysis, however, would map the prefix *un-* and the negation particle *not* to proposition-level negation ($\neg$) of standard logic.  -->
...
<!-- @orwell1946politics voiced this opinion. -->


<!-- morphological negation via affixation (e.g., *un-*)  -->

<!-- Language provides multiple ways of conveying the opposite, and there is little agreement on what to do when they combine.  -->
<!-- A person could theoretically be *happy*, *not happy*, *unhappy*, and perhaps even *not unhappy*.  -->
...
Both syntactic [@Cable2017] and pragmatic [@Rett2014:eval] mechanisms have been proposed, but heretofore there have been no computational accounts tested against human behavioral data.
...
We hypothesize that *lexical uncertainty*---uncertainty in the meaning of words---pervades even the language of logical devices, which interacts with conversational reasoning to give rise to a panoply of context-specific interpretations.
Specifically, we posit that overt negation markers ("not", "un-") can convey either contrary or contradictory opposition and that listeners resolve their uncertainty about the meaning in context.

Formally, a gradable adjective (e.g., "happy") is thought to be literally true if the degree associated with that adjective $x$  (e.g., the degree of happiness) is greater than some contextually-determined threshold $\theta_1$: $\denote{happy}(x, \theta): x > \theta_1$ [@Kennedy2007].
If a negation marker (e.g., "not") creates a contradictory statement, then the resulting meaning would simply be that the degree is less than or equal to the threshold: $\neg \denote{happy}(x, \theta_1): x \leq \theta_1$.
Alternatively, negation could create a *contrary* meaning, where a new predicate is formed and which is assigned its own threshold $\theta_2$: $\denote{\tilde{happy}}(x, \theta_2): x < \theta_2$.
\mht{i think this point could be moved to a model implementation appendix or footnote}
Contradictory opposition can be iterated ($\neg \neg Hx$) but contrary opposition cannot [@Horn1989:Natural].
As a result, a single negative ("not happy" and "unhappy") can mean either $\neg happy$ or $\tilde{happy}$, while double negatives ("not unhappy") may mean $\neg \neg happy$ or $\neg \tilde{happy}$ (Fig.$\thinspace$\ref{fig:lexicon-model}).


<!-- Formally, if $Hx$ expresses that $x$ is happy, contradictory opposition is standard, proposition-level negation $\neg Hx$: $Hx$ and $\neg Hx$ is a contradiction. -->
<!-- Contrary opposition, on the other hand, forms a new predicate $\tilde{H}x$ which introduces its own threshold $\theta_{\tilde{H}}$. -->
<!-- Thus, we propose a model where overt negation markers (*un-*, *not*) could be mapped to either oppositional meaning---contrary or contradiction---up to the constraints of compositionality (full *uncertain negation* model). -->



<!-- The positive adjective "happy" is literally true if the degree of happiness is greater than some contextually-determined standard $\theta$,  -->
<!-- pragmatic listener as having uncertainty  -->
<!-- Since gradable adjectives like "happy" are vague and context-dependent,  -->

In order to generate predictions, we formalize this *uncertain negation* hypothesis in a probabilistic model of pragmatic reasoning in the Rational Speech Act tradition [@Franke2015a; @Goodman2016:RSA], a recursive reasoning model wherein a pragmatic listener $L_{1}$ tries to resolve the intended meaning of an utterance $u$ (e.g., "Jones is not unhappy") by combining its prior beliefs about the degree of Jones' happiness $P(x)$ with the Gricean assumption that speakers are generally cooperative $S_1$ (Eqs.$\thinspace$\ref{eq:L1}-\ref{eq:L0}):
Listener uncertainty about the interpretation of negation markers is modeled as uncertainty about the speaker's lexicon $\mathcal{L}$ [@Bergen2016].
We combine this technique with the model of @Lassiter2015 that derives thresholds $\theta$ for interpreting gradable adjectives (e.g., happy) in context:
...
where $\mathcal{L}(u, x, \theta)$ gives the truth-value of $u$ in lexicon $\mathcal{L}$ applied to state $x$ under threshold $\theta$ (e.g., for $u= $"happy", determining whether or not $x>\theta$).
The pragmatic listener (Eq. \ref{eq:L1}) has uncertainty about $\theta$, which comes from an uninformed prior $P(\theta)$ [following @Lassiter2015] and is resolved by jointly reasoning about the likely degree $P(x)$, the likely lexicon $P(\mathcal{L})$, and the likelihood $S_1(u \mid x, \theta, \mathcal{L})$ that a cooperative information-maximizing speaker would utter the adjective given a degree $x$, threshold $\theta$, and lexicon $\mathcal{L}$.

```{r lexicon-model, fig.pos = "t", fig.cap="Space of possible meanings in the lexicon prior for the  \\emph{logical negation}, \\emph{bonafide contraries}, and the full \\emph{uncertain negation} models.", cache = F}
# ggdraw() + draw_image("img/lexicon-model2.pdf", scale = 1)+
#   theme(plot.margin = unit(c(2,0,0,0), "pt"),
#         panel.background = element_rect(colour = "black", size=1))

img <- png::readPNG("img/lexicon-model2.png")
grid::grid.raster(img)
```


<!-- Interpretations of antonym pairs and their negations depend on the meanings a listener considers in their prior distribution over possible lexica $P(\mathcal{L})$. -->
<!-- Our concern in this work is on negation and thus we consider possible meanings of negation .  -->

<!-- positive adjectives (*happy*), antonyms (*unhappy*), negated positives (*not happy*) and negated antonyms (*not unhappy*) -->
<!-- Predictions depend on the space of possible meanings considered in the lexicon prior $P(\mathcal{L})$. -->
We compare the predictions of this uncertain negation model to three alternative models, which highlight different questions of theoretical interest.
First, we compare to a vanilla RSA model such as that in @Frank2012, which has neither lexical uncertainty or threshold uncertainty.
The vanilla model assumes fixed-thresholds to the adjectives, and we assume antonyms convey contrary meanings (e.g., "happy" means $>70\%$ on the happiness scale; "unhappy" means $<30\%$).
Second, we compare to the uncertain threshold model of @Lassiter2015, assuming that all negation marker entail contradictory negation (the *logical negation* or *George Orwell* model).
Finally, we construct a *bonafide antonyms* model, also following @Lassiter2015, which assumes threshold uncertainty but where morphological antonyms (*un-*) convey contrary negation.

<!--
Each of the possible meanings for negation markers described above results in a different lexicon $\mathcal{L}$ and the space of possible meanings forms a prior distribution over possible lexica $P(\mathcal{L})$
We consider two other natural possibilities for the space of meanings in order to derive alternative models.
One alternative is that negation markers map only onto contradictory meanings (the *logical negation* or *George Orwell* class of meanings; Fig.$\thinspace$\ref{fig:lexicon-model} blue dashed lines).
Alternatively, morphological negation (*un-*) could express a bonafide contrary meaning (a la lexical antonyms like *sad*; Fig.$\thinspace$\ref{fig:lexicon-model} purple fuzzy lines).^[
  For maximal similarity to the full *uncertain negation* model, we allow the *bonafide antonyms* model to maintain uncertainty about particle negation (*not*).
  Instead, in the *bonafide antonyms* model, one could fix the meaning of particle negation (*not*) to be logical negation.
  This choice turns out to not produce any qualitative difference from maintaining uncertainty about *not*'s meaning.
<!-- The model infers that *not* should be treated as logical negation, given the other constraints in the lexicon. -->
]
-->
...
<!-- These differences are shown to be sensitive to the presence of explicit alternative utterances, as suggestsed by Krifkas's example above.  -->
...
<!-- Formal linguistic theories capture the meaning of gradable adjectives as a threshold function:  $\denote{happy} = \lambda x \thinspace . \thinspace \text{happiness}(x) > \theta$, whose threshold variable $\theta$ is supplied by the context [@Kennedy2007].  -->
<!-- Here, we introduce contrary and contradictory negations into a pragmatic model that reasons about a speaker's likely $\theta$ [@Lassiter2015].  -->

Upon hearing "not unhappy", our *uncertain negation* model reasons that a truly compositional $\neg \neg \textit{happy}$ is implausible because the speaker could have just said the simpler "happy".^[
Model predictions use the following minimally assumptive model parameters: $P(x) = \mathcal{N}(0, 1); \alpha = 1; \text{cost}(\mathit{un}) = 1 < \text{cost}(\mathit{not}) = 2$.
Predictions are qualitatively similar when $\text{cost}(\mathit{un}) = \text{cost}(\mathit{not})$.
]
interprets the utterance as signalling a slightly positive state (Fig.$\thinspace$\ref{fig:modelPredictions})
At the same time, the uncertain negation model does not differentiate  "unhappy" (antonyms) from "not happy" (negated positives), as @Jespersen1917:Negation and @Blutner2004:pragmatics surmised.

The model draws these inferences by reasoning about which lexicon best explain's a speaker's single utterance (Fig.$\thinspace$\ref{fig:modelPredictions}, *single utterance*).
...
The formal modeling approach we take hear naturally allows for this extension by conditioning $L_1$ on the observation of a speaker using multiple adjective alternatives to describe different referents (e.g., "John is not happy. Bill is unhappy"; Fig.$\thinspace$\ref{fig:modelPredictions}, *multiple utterances*).
When the model hears multiple utterances in the same context, the model does indeed predict a meaning difference between "unhappy" and "not happy": "unhappy" is more sad than "not happy", producing the ordering hypothesized by @Krifka2007:Negated-antonyms in Fig.\ref{fig:happy-scale}.
<!-- Multiple utterances heard in the same context, however, can help resolve the meaning of each other, and thus yield different interpretations than if the utterances were heard in isolation. -->

This pattern of judgments is uniquely predicted by the *uncertain negation* model.
The *bonafide contraries* models also yields interpretations of negated antonyms as slightly positive, but predicts Krifka's ordering for both single and multiple utterance conditioning.
...
The experiment took on average `r round(d.expt1.time.summary[[1, "aveTime"]],0)` minutes and participants were compensated \$0.40.
...
10 adjective sets were constructed for each antonym type (total 20) from an informal survey of the linguistics literature and taken from a list of "common opposites" available online (Table 1).^[http://www.enchantedlearning.com/wordlist/opposites.shtml]
...
Closer investigation of responses revealed that negated antonyms (and not other adjective types) received a bimodal distribution: Most ratings were slightly positive but a clearly distinguishable minority distribution of ratings were slightly negative (e.g., *not dishonest* meaning *not honest*).
...
The experiment comprised of four between-subjects experimental conditions arranged in a 2x2 design: *antonym type (morphological vs.\text{~}lexical)* X *context (single vs.\text{~}multiple utterances)*.
300 participants were assigned to each *antonym type* in the *single utterance* contexts, and 75 participants were assigned to each in the *multiple utterances* conditions.
These numbers follow from the intention of getting approximately 45 ratings for each unique adjective in the experiment.
The *single utterance* task took on average `r round(filter(d.time.summary, condition == "implicit")[[1,"aveTime"]], 0)` minutes and participants were compensated \$0.40; *multiple utterances* took on average `r round(filter(d.time.summary, condition == "explicit")[[1,"aveTime"]], 0)` minutes and participants were compensated \$0.80.
...
To best isolate the contribution of morphological vs.\text{~}lexical antonyms, we curated adjective sets consisting of words for properties of people, such that both types of antonyms existed for the same positive adjective (e.g., *happy* $\rightarrow$ *unhappy*, *sad*; \text{Table$\thinspace$2}).
...
The *single utterances* conditions were similar to that of \text{Expt.$\thinspace$1}: Participants rated one sentence at a time (e.g., "Greg is not unhappy"), each from a unique adjective set (e.g., never rated both *unhappy* and *not happy*), completing a total of 12 trials, with exactly 3 repetitions of each adjective type (positive, antonym, and their negations).
...
As we did in \text{Expt.$\thinspace$1}, we evaluate our hypothesis that morphological antonyms behave like the *uncertain negation* model (i.e., show a partial ordering) while lexical antonyms show a true ordering (like *bonafide contraries*).
We considered data only from the *single utterances* conditions and built a linear mixed model predicting the unnormalized ratings in terms of *antonym type* (morphological vs.\text{~}lexical), *adjective type* (Helmert coded in order: antonym, negated positive, negated antonym, positive) and their interaction; the model also included random intercepts and random slopes of *adjective type* by-participant and by-item.
Consistent with our hypothesis, the interaction between the *antonym* vs.\text{~}*negated positive* levels of adjective type and antonym type (morphological vs.\text{~}lexical) was significant ($\beta = `r round(rs1.implicit.coef["antonym_typelexant:adj_type1","Estimate"],3)`$, t$(`r round(rs1.implicit.coef["antonym_typelexant:adj_type1","df"], 0)`) = `r round(rs1.implicit.coef["antonym_typelexant:adj_type1","t value"],2)`, p = `r round(rs1.implicit.coef["antonym_typelexant:adj_type1","Pr(>|t|)"], 4)`$).
We then analyzed the simple effects.
Morphological antonyms were not significantly different than negated positives ($\beta = `r format(rs1.implicit.simple.morph.coef["adj_type1","Estimate"], digits = 2, scientific = T)`$, t$(`r round(rs1.implicit.simple.morph.coef["adj_type1","df"], 0)`) = `r round(rs1.implicit.simple.morph.coef["adj_type1","t value"],2)`, p = `r round(rs1.implicit.simple.morph.coef["adj_type1","Pr(>|t|)"], 2)`$), while lexical antonyms were interpreted more negatively than negated positives ($\beta = `r -1*round(rs1.implicit.simple.lex.coef["adj_type1","Estimate"],3)`$, t$(`r round(rs1.implicit.simple.lex.coef["adj_type1","df"], 0)`) = `r round(rs1.implicit.simple.lex.coef["adj_type1","t value"],2)`$,
...
This interaction was also significant ($\beta = `r round(rs2.morph.coef["conditionexplicit:adj_type1","Estimate"],3)`$, t$(`r round(rs2.morph.coef["conditionexplicit:adj_type1","df"], 0)`) = `r round(rs2.morph.coef["conditionexplicit:adj_type1","t value"],2)`, p = `r format(rs2.morph.coef["conditionexplicit:adj_type1","Pr(>|t|)"], digits = 3, scientific = T)`$), and in the correct direction (see Fig.\text{~}\ref{fig:expt-results}B).
...
This work provides a computational solution to an outstanding puzzle in natural language understanding: How to interpret double negatives [e.g., *not unhappy*; @Krifka2007:Negated-antonyms; @Rett2014:eval].
We additionally discovered and confirmed a surprising empirical result that challenges ``established'' intuitions in linguistics: *unhappy* and *not happy* are not immediately differentiated, except when both are present in the same context.
Our model that represents uncertainty about how to interpret overt negation markers (*un-*, *not*) predicts this very result, while alternative models that treat negation with a fixed meaning fall short.
One limitation of this work is that we stipulate, rather than derive, differences in meaning for morphological vs. lexical antonym pairs [cf., @Rett2014:eval].
...
The two types of negation meaning we considered---contrary and contradictory opposition---can be seen as a *lexicalized* form of opposition (with the adjective receiving its own threshold variable) and a *compositional* rule (logical negation), respectively.
In our modeling, we assumed all lexica (all logically-possible interpretations of negation) were equally likely *a priori*: A further test of our negation uncertainty model would be to see if frequency can serve as a proxy for this prior over lexica.
