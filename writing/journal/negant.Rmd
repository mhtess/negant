...
With language, however, it's less clear what should happen:
<!-- In language, however, it is than clear what should happen with double negatives. -->
...
This intuition is not uncontroversial (e.g., Orwell's quote above), nor is there agreement about how a listener derives a weakly positive itnerpretation from a double negative.
...
The key fact about contaries is that they can both be false (unlike *contradictions*): Jones may be neither tall nor short (contraries) but a natural number cannot neither be even nor odd [contradictions; @Horn1989:Natural].
...
Such an analysis, however, further depends on the meaning of "not happy" [@Blutner2004:pragmatics], of which there is also little agreement: @Jespersen1917:Negation and @Blutner2004:pragmatics argue that "not happy" means the same as "unhappy", while @Krifka2007:Negated-antonyms cites examples like: 
...
Both syntactic [@Cable2017] and pragmatic [@Rett2014:eval] mechanisms could be at play, but heretofore there have been no computational accounts tested against human behavioral data.
We hypothesize that *lexical uncertainty*---uncertainty in the meaning of words---pervades even the language of logical devices, which interacts with pragmatic reasoning to give rise to a panolpy of context-specific interpretations.
...
Literally, a gradable adjective (e.g., "happy") is thought to be true if the degree associated with that adjective $x$  (e.g., the degree of happiness) is greater than some contextually determined threshold $\theta_1$: $\denote{happy}(x, \theta): x > \theta_1$ [@Kennedy2007].
...
Alternatively, negation could create a *contrary* meaning, where a new predicate is formed that gets assigned its own threshold $\theta_2$: $\denote{\tilde{happy}}(x, \theta_2): x < \theta_2$.
Contradictory opposition is iterable ($\neg \neg Hx$) but contrary opposition is not [@Horn1989:Natural].
...
Each possible meanings for negation results in a different lexicon $\mathcal{L}$ and the space of possible meanings can be considered as a prior distribution over possible lexica $P(\mathcal{L})$
...
We combine this technique with the model of @Lassiter2015 that derives plausible thresholds $\theta$ for gradable adjective (e.g., happy) interpretation:
...
Listeners reason about which lexicon best explains a speaker's single utterance (Fig.$\thinspace$\ref{fig:modelPredictions}, *single utterance*).
If a speaker uses multiple utterances in the same context, the listener may have even more information about the speaker's lexicon.
...
We thus also generate predictions for our models by conditioning $L_1$ on the observation of a speaker using all four adjective alternatives to describe four different individuals in the same context (Fig.$\thinspace$\ref{fig:modelPredictions}, *multiple utterances*).
