...
In logic, two negatives make a positive, but with words it's less clear what should happen.
...
In other words, *negated antonyms* (e.g., "not unhappy") could occupy a particular region of their associated scale (e.g., happiness), below  *positive adjectives* ("happy") but above *negated positives* ("not happy") and *antonyms* ("unhappy") [Fig.$\thinspace$\ref{fig:happy-scale}; @Krifka2007:Negated-antonyms].
Not everyone agrees with this intuition (e.g., Orwell's quote above), nor is there agreement about how to go from two negatives to a weak, positive interpretation.
An antonym created by morphology (*morphological antonyms* e.g., "un"+"happy") could convey *contrary opposition*, which is often conveyed by antonyms which have their own lexical entry (*lexical antonyms* e.g., "sad").
...
In a pragmatic analysis, "not unhappy" would then compete with "happy" as an alternative description and would be contextually strengthened to a more specific interpretation, namely to a neutral or indifferent state [@Horn1991:Duplex], contra @Jespersen1924's intuition that "not unhappy" is a slightly positive state.
...
We propose that *lexical uncertainty*---uncertainty in the meaning of words---pervades even the language of logical devices, which interacts with pragmatic reasoning to give rise to a panolpy of context-specific interpretations.
Specifically, we posit that overt negation markers ("not", "un-") can convey either contrary or contradictory opposition, and that listeners retain uncertainty about the intended meaning *a priori*.

We formalize this *uncertain negation* hypothesis in a probabilistic model of pragmatic reasoning in the Rational Speech Act tradition [@Franke2015a; @Goodman2016:RSA], a recursive reasoning model wherein a pragmatic listener $L_{1}$ tries to resolve the intended meaning of an utterance $u$ (e.g., "Jones is not unhappy") by combining its prior beliefs about the degree of Jones' happiness $P(x)$, with the generative process of the utterance, a speaker model $S_1$ (Eqs.$\thinspace$\ref{eq:L1}-\ref{eq:L0}):
...
Any predicted qualitative differences between interpretations of antonym pairs and their negations will depend on the space of meanings considered in the lexicon prior $P(\mathcal{L})$.
...
One is that negation markers map only onto contradictory meanings (the *logical negation* or *George Orwell* class of meanings; Fig.$\thinspace$\ref{fig:lexicon-model} blue dashed lines).
Alternatively, morphological negation (*un-*) could express a bonafide contrary meaning (a la *sad*; Fig.$\thinspace$\ref{fig:lexicon-model} purple fuzzy lines).^[
  ...
  This turns out to not produce any qualitative difference from maintaining uncertainty about *not*'s meaning. 
...
For a given lexicon prior, listeners reason about which lexicon best explains a speaker's single utterance (Fig.$\thinspace$\ref{fig:modelPredictions}, *single utterance*).
