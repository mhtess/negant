...
\shorttitle{SOM-R: Flexible negation in language}
...
We also describe two alternative pragmatics hypotheses, one inspired by \citeA{Horn1989:Natural} and another investigating the role of the Question Under Discussion (or, QUD). 
...
We study negation in the context of gradable adjectives because their degree-based semantics allows us to differentiate the meaning of negation markers when interpreted as either contradictory or contrary negation.\footnote{We use the term \emph{contrary} to refer to \emph{polar contraries}. ``black'' and ``blue'' are also contraries (sometimes called \emph{simple} or \emph{reductive contraries}, but they do not refer to opposite poles of the same scale \cite{Horn1989:Natural}}
...
Context-sensitivity falls out of the prior distribution over world states $P(x)$ (a basic component in any RSA model); in this case, it is a prior distribution over degrees (e.g, prior expectations about the heights of buildings). 
...
The Aristotelean model assumes that \emph{happy} and \emph{unhappy} are contraries, which each gets assigned their own truth conditional threshold (e.g., $\theta$ and $\theta'$). Thus, instead of the pragmatic listener defined by Equation \ref{eq:L1}, we have: 
%
\begin{align}
...
\caption{QUD based models for the single utterance condition. The listener reasons that if the Question Under Discussion were a polar QUD (``is John happy?''), the speaker would only give a polar response (``happy'', ``not happy''). The listener then infers the QUD given an utterance, leading to a stronger (more negative and more positive) interpretations for the utterances with negation (``not happy'', ``not unhappy'', respectively).}.\label{fig:qud}
\end{figure}
...
\begin{figure}[t]
\centering \includegraphics{figs/expt3b_byItem_densities.pdf}
\caption{Results by item for Experiment 3b.}.\label{fig:items_expt3}
\end{figure}
