...
The normalization in Eq.~\ref{eq:S1} implies a normalization of over a set of alternative utterances (which potentially differ in their cost). 
We assume in all models that the set of alternative utterances is the positive adjective, the antonym, and their corresponding particle negations (e.g., happy, unhappy, not happy, not unhappy). 
...
Contrary negation as a predicate-forming operation (as we described in the section on Formal Semantic Representations) discards information about the original threshold $\theta$ of the embedding predicate (e.g., if \emph{unhappy} is a contrary, it receives a different, independent threshold from that of \emph{happy}).
...
This distributional restriction for contraries introduces a complication when dealing with negated morphological antonyms (\emph{not un-})  under lexica in which \emph{not} is a contrary. 
% when (the outer negation) \emph{not} is a contrary because 
 The distributional restriction described above would render this utterance impossible for this kind of lexicon.
...
With these assumptions, the model derives the key ordinal predictions when the cost of morphological negation is less than the cost of particle negation  (Figure \ref{fig:flexNeg_variants}, left two facets), but in the next section, we describe another way of breaking the symmetry.

\paragraph{Symmetry breaking}
%
The symmetry of morphological vs. particle negation introduced by the \ourmodel model can be broken via either cost asymmetry (as we report in the main text) or the structure of the lexicon prior. 
In particular, if we interpret the distributional restriction on contraries (described above) as not only constraining the space of possible meanings given certain lexica, but constraining the space of lexica itself, then the ``both contraries'' lexicon would be rendered invalid (or perhaps more generously, low probability). That is, you can have either \emph{not} or \emph{un-} as contraries or they may both be contradictions,  but they cannot both be contraries. 
...
interprets the utterance as signaling a slightly positive state (Figure \ref{fig:modelPredictions}).
...
The formal modeling approach we take here naturally allows for this extension by simply providing \(L_1\) with multiple adjective phrases; we condition each model on the observation of a speaker using all four adjective alternatives to describe different referents (e.g., \emph{Sue is happy. Steve is not happy. Bill is unhappy. Barb is not unhappy.}; Figure \ref{fig:modelPredictions}, \emph{multiple utterances}).
...
\subsubsection{Varying alternatives model (\emph{a la} Horn, 1989)}


%\begin{figure}[t]
%\centering \includegraphics{figs/hornModels.pdf} 
%\caption{Varying alternatives-based models inspired by Horn (1989).}.\label{fig:horn}
%\end{figure}

\subsubsection{QUD-based models}

The interpretation of negated utterances often hinges critically on contextually available alternative utterances that can define the ``Question Under Discussion'' (or, QUD), the implicit communicative goal that the speaker is trying to achieve. 
For example, if a speaker asks, ``How is Mary doing?'' and you reply that she is ``not unhappy'', the intended meaning might be different than if the speaker asks ``Is Mary unhappy?'' and you give the same reply.
Expressions involving negation might be preferentially deployed when addressing Polar QUDs.
In all of  our models, we have assumed a ``state'' QUD (analogous to the first question: ``How is Mary doing?''). 
However, since we do not control the prosodic stress placed on utterances in our study (because they are text-based studies) and stress can modulate the QUD, it remains a possibility that some kind of QUD-inference could be occurring that would lead to the pattern of results we observe in our experiments. 

We present four QUD-based models.
Three of these simply look at the effect of QUDs on the model.
The fourth articulates a generative model of speaker utterances based on different QUDs and formalizes how a listener should invert this generative model to simultaneously infer the QUD as well as report on the state (e.g., the degree of happiness). 

For the first three, we stipulate that hearing a negation particle (``not'') changes the QUD to a polar QUD (e.g., hearing ``not happy'' causes the listener to believes that QUD is \emph{Is John happy?}). 
The first of these models assumes this occurs in a strict, deterministic manner, whereas the other two encode this relationship in a probabilistic manner.
...
The asymmetric version assumes the following probabilities:  $not \rightarrow P(\text{polar QUD}) = 0.8$ otherwise, $P(\text{polar QUD}) = 0.5$. 
The asymmetric version assumes:  $not \rightarrow P(\text{polar QUD}) = 0.8$ otherwise, $P(\text{polar QUD}) = 0.2$. 
...
The QUD inference model is more elaborate and one perhaps that will serve as a starting point for future research on this topic. This model assumes if a speaker is trying to address a polar QUD, they will only say effectively “yes” or “no” (e.g., “Is John happy?”... speaker can say “happy” or “not happy”). The pragmatic listener reasons about the QUD by considering this model of speaker behavior; thus, when they hear “John is not happy”, they are more likely to believe the question was in response to a polar QUD (Is John happy?). 
This model is styled after the QUD inference model introduced by Kao et al. (2014) but goes beyond it in articulating how the QUD changes the alternative utterances available to the speaker. 
