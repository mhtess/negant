...
But what about "Jones is not unhappy"? 
Do the two negatives make a positive?
@Jespersen1924 believed not:

> The two negatives do not exactly cancel one another... the longer expression is always
> weaker: "this is not unknown to me" or "I am not ignorant of this" means *I am to some extent
> aware of it*, etc. (p.832)
...
A *contrary opposition*, such as between "happy" and "sad", is one where both predicates cannot be true at the same time, but both can be false. 
A *contradictory opposition*, such as between "even" and "odd", is one where falsity of one predicate entails truth of the other. 
In other words, contrary opposites about gradable terms allow for a neutral middle ground, unlike contradictory opposites. 
In language, *negative-form adjectives* (e.g., "sad", "rude") intuitively express contrary opposition to positive-form adjectives ("happy", "polite"): Jones is allowed to be neither happy nor sad.
Other meaning-opposing linguistic devices ---*particle negation* (e.g., "not happy") and *antonyms* (e.g., "unhappy")--- could then denote contradictory opposition.
...
> formation... It should be possible to laugh the "not un-" formation out of existence by
> memorizing this sentence: "A not unblack dog was chasing a not unsmall rabbit across a not
> ungreen field." \mf{pageref}

Instead, antonyms created by affixal negation ("un-") could form a contrary, like a negative adjective.
...
"Not happy" seems like it's also an alternative [@Blutner2004:pragmatics], but how pragmatics contextually strengthens "not happy" has not been fully spelled out [@Krifka2007:Negated-antonyms].
Furthermore, the meaning of "not happy" is at least as controversial as "unhappy": @Jespersen1917:Negation and @Blutner2004:pragmatics argue that the two are basically identical in meaning (i.e., affixal and particle negation are both contradictory in nature), while @Horn1989:Natural and @Krifka2007:Negated-antonyms disagree.

Given this indeterminacy in meaning of particle negation ("not") and affixal antonyms ("un-"), we hypothesize that listeners too have uncertainty about how to interpret overt negation markers and that this uncertainty is resolved in context.
We extend previous work deriving interpretations for gradable adjectives by considerations of cooperative language use [@Lassiter2015; @Qing2014:Adjectives] to incorporate compositional expressions involving various forms of negation: how happy or sad is an agent who is reported to be *not happy*, *not sad*, *unhappy* or *not unhappy*? 
We propose a probabilistic speaker-listener pragmatic reasoning model in the Rational Speech Act tradition [@Franke2015a; @Goodman2016:RSA], introducing elements of lexical uncertainty [@Bergen2016] in order to model the indeterminacy about how to interpret overt negation markers.
We investigate the empirical phenomena in three experiments that measure interpretations for
positive- and negative-form adjectives ("happy"/"sad") as well as affixal antonyms and particle negations in different contexts, uncovering subtle but reliable differences.
These findings suggest that pragmatic reasoning can utilize the uncertainty in logical meanings
to derive finer-grained interpretations by contrasting multiple interpretations of multiple
alternative utterances simultaneously.
\mht{this last sentence is a behemoth}
...
Their meanings can be modeled using a threshold function: $\denote{happy} = \lambda x \thinspace . \thinspace \text{happiness}(x) > \theta$, whose threshold variable $\theta$ is supplied by the context [@Kennedy2007].
...
Eqs.$\thinspace$\ref{eq:L1}-\ref{eq:L0} are a Rational Speech Act (RSA) model, a recursive Bayesian model wherein a pragmatic listener $L_1$ tries to resolve the intended meaning of an utterance $u$ by combining their prior beliefs $P(x)$ with the generative process of the utterance, a speaker model $S_1$.
...
The literal listener $L_0$ updates their prior beliefs $P(x)$ via an utterance's literal meaning $[\![u]\!](x)$, the threshold function described above.
The pragmatic listener has uncertainty about $\theta$, which comes from an uninformed prior and is resolved by jointly reasoning about the likely states of the world $P(x)$ (e.g., possible degrees of happiness) and the likelihood $S_1(u \mid x, \theta)$ that a speaker would say the adjective given a state $x$ and a threshold $\theta$.

\begin{table}[b]
  \centering
  \begin{tabular}{ll}
    \emph{not happy} / \emph{unhappy} & $\lambda x \thinspace . \thinspace \neg (\mathit{happiness}(x) >
    \theta_{h})$ \\
    & $\lambda x \thinspace . \thinspace \mathit{happiness}(x) <
    \theta_{\tilde{h}}$ \\ \hline
    \emph{not unhappy} & $\lambda x \thinspace . \thinspace \mathit{happiness}(x) >
    \theta_{h}$ \\
...
We posit that listeners have uncertainty about whether each negation marker ("not", "un-") is contradictory or contrary but maintain structural constraints on composition.
If $Hx$ expresses that $x$ is happy, contradictory opposition is standard bivalent negation $\neg Hx$.
...
We consider the set of all combinations of logically possible interpretations of negated expressions (Table$\thinspace$\ref{tab:Interpretations}) as the set of possible lexica, and assume, for simplicity, that the listener has treats all lexica as equally likely *a priori*.
...
\noindent where $\mathbf{\theta}$ is a now vector of relevant thresholds.
...
<!-- With the alternative set \{"happy", "not happy", "unhappy", "not unhappy"\}, treating *unhappy* as the antonym of *happy*,  -->
This model does not differentiate between negated antonyms ("not unhappy") and positive statements ("happy"), nor negated positives and antonyms.
<!-- \mht{i like using "negated antonyms" and "negated positives", etc. but we should introduce this terminology earlier... i believe this is the first usage} -->
...
Even though this model has *a priori* uncertainty about the correct interpretation of particle negation ("not"), pragmatic reasoning infers that "not" should be interpreted as logical negation so as to keep the lexicon non-redundant.
...
Finally, the *uncertain parser* model considers the full set of logically possible lexica.
...
\mht{either explain the reasoning behind "parse" or rename model}

\mht{i think we could get a prediction for expt 2 / expt 3 (expl. vs. impl. alternatives) by conditioning the uncertain parsing model on all 4 (or at least more than 1) utterance, while keeping the lexicon fixed. for instance, to interpret "not unhappy", the listener must ascribe "not" as logical negation and "un-" as contrary... if the listener must fit all the utterances to the same lexicon, that should make it more like the bonafide antonyms model...}

To summarize, we hypothesize that supposed antonyms created by affixal negation have uncertainty in parsing and thus will produce the partial ordering observed in Figure$\thinspace$\ref{fig:modelPredictions} for the *uncertain parse model*.
Lexically distinct antonyms, on the other hand, we predict will exhibit the complete ordering predicted by the *bonafide antonyms* model.^[
For this prediction, we would assume the alternative set includes the lexically-distinct antonym e.g., \{"sad", "not happy", "not sad", "happyy"\}.
]
