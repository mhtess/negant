...
*Not unhappy*, then, would compete with *happy* as pragmatic alternative and so would be pragmatically strengthened to a more specific interpretation, namely to a neutral or indifferent state [@Horn1991:Duplex], contra @Jespersen1924's intuition that *not unhappy* is a slightly positive state.
...
These differences are shown to be sensitive to the presence of explicit alternative utterances [as illustrated in Krifkas's example above]. 
...
Does this model predict that $L_1$ would infer any qualitative differences between positive (*happy*), antonym (*unhappy*), negated positive (*not happy*) and negated antonym (*not unhappy*)?
...
When listeners reason about which lexicon best explained the observed speaker utterance, it matters whether only a single utterance or multiple utterances are observed from the same speaker.
...
Upon hearing a single utterance of  *not unhappy*, the *uncertain negation* model reasons that a truly compositional $\neg \neg \textit{happy}$ is implausible (intuitively because the speaker could have said the simpler *happy*) and interprets the utterances as signalling a slightly positive state.^[Model predictions assume the following model parameters: $P(x) = \mathcal{N}(0, 1); \alpha = 1; \text{cost}(\mathit{un}) = 1; \text{cost}(\mathit{not}) = 2$.]
Under single utterance conditioning, the *uncertain negation* model does not differentiate antonyms (*unhappy*) from negated positives (*not happy*), as @Jespersen1917:Negation and @Blutner2004:pragmatics surmised.
But multiple conditioning, the model predicts that *unhappy* is more happy than *not happy*.
The *bonafide contraries* submodel interprets negated antonyms (*not unhappy*) as slightly positive and predicts the ordering hypothesized by @Krifka2007:Negated-antonyms as shown in Fig.\ref{fig:happy-scale} for both single and multiple utterance conditioning. 
The *logical negation* model does not differentiate between negated antonyms (*not unhappy*) and positives (*happy*), nor between negated positives and antonyms.

All models have more extreme interpretations in the multiple conditioning case.
...
The *uncertain negation model* predicts that a partial ordering for antonyms and their negations when heard in isolation, but a full ordering when present in the same context (Fig.$\thinspace$\ref{fig:modelPredictions}).
...
Closer investigation of responses revealed that negated antonyms received a bimodal distribution: The vast majority of ratings were slightly positive, while a clearly distinguishable minority distribution of ratings were slightly negative (e.g., *not dishonest* for a person below average in honesty).
...
\vspace{1em} \fbox{\parbox[b][][c]{7.3cm}{\centering \footnotesize Experimental paradigms, computational models, and data for this paper can be found at \url{https://mhtess.github.io}.
}} \vspace{1em}
