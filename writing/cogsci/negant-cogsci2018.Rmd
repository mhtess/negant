...
A *contradictory* opposition is one where falsity of one predicate entails truth of the other  (e.g., *even/odd* positive integer).
...
Here, we combine a pragmatic model that reasons about values for $\theta$ that would make a speaker's utterance most likely [@Lassiter2015] with contrary and contradictory negations. 
...
We explore the simplest instantiation of this *uncertain negation* model, putting equal probability on the logically possible parses of utterances with negation.^[Model predictions assume the following model parameters: $P(x) = \mathcal{N}(0, 1); \alpha = 1; \text{cost}(\mathit{un}) = 1; \text{cost}(\mathit{not}) = 2$.]
When this model hears *not unhappy* it reasons that a truly compositional, double negation is implausible (intuitively because the speaker could have said the simpler *happy*) and interprets the utterances as signalling a slightly positive state (Fig.$\thinspace$\ref{fig:modelPredictions}).
...
We generate alterantive hypotheses by perturbing the lexicon prior $P(\mathcal{L})$.
...
As such, multiple utterances heard in the same context can have different meanings than if the utterances are heard in isolation. 
...
The *uncertain negation* model, though, uniquely differentiates itself from the single utterance simulation: It now predicts a true ordering as in Fig.$\thinspace$\ref{fig:happy-scale} between negated positives and antonyms.
...
fig.expt1 <- ggplot(df.oneSlider.ci %>%
    ...
    geom_linerange(size = 10, alpha = 1)+
    geom_linerange(data = df.oneSlider.ci %>%
           filter(antonym_type == "morphological" & adjective_type %in% c("Negated positive", "Antonym")),
           position = position_dodge(width = 0.02), size = 10, inherit.aes = F,
           aes(x = condition_numeric,
                      y = mean,
                       ymin = ci_lower, ymax = ci_upper,
                       color = new_type))+
  ...
  #     jitter.width = 0), size = 10, alpha = 1)+
    ...
    geom_text(x = -0.09, y = -0.75, label = 'SINGLE UTTERANCE', color = 'black', alpha = 0.8, family = "Palatino",
              fontface = 'italic', size = 4)+
    # geom_text(x = 0.15, y = -0.85, label = 'morphological antonyms', color = 'darkblue', size = 4, family = "Palatino")+
    # geom_text(x = -0.05, y = -0.95, label = 'lexical antonyms', color = 'black', alpha = 0.6, size = 4, family = "Palatino")+
    # geom_text(x = -0.09, y = 1, label = 'SINGLE UTTERANCE', color = 'black', alpha = 0.8, family = "Palatino",
    #           fontface = 'italic', size = 4)+
    scale_color_manual(name="Adjective type",
                    #values = c("#74a9cf", "#0570b0","#5e3c99",  "#e66101", "#b2abd2","#fdb863"),
                    values = c("#5e3c99",  "#0570b0",  "#b2abd2", "#74a9cf","#fdb863","#e66101"),
                    #values = c("#5e3c99",  "#0570b0","#e66101",  "#b2abd2", "#74a9cf","#fdb863"),
                    guide = guide_legend(reverse=F),
...
fig.expt2.multiple.legend <- d.full.boot %>%
  ungroup() %>%
  ...
  ggplot(., aes(x = 0,
    ...
    xlab("")+
    coord_flip()+
    geom_linerange(position = position_jitterdodge(dodge.width = 0.3, jitter.height = 0, jitter.width = 0), size = 3, alpha = 1)+
    ...
    scale_color_manual(name="Adjective type",
                    values = orange.purple.blue.color.palette,
                    guide = guide_legend(nrow = 2, reverse=F),
...
fig.expt2.single <- ggplot(df.expt2.single %>%
    ...
    geom_vline(xintercept = 0.2,  size = 5, alpha = 0.1, color = 'darkblue') +
    geom_linerange(size = 10, alpha = 1)+
    geom_linerange(data = df.expt2.single %>%
           filter(antonym_type == "morphological" & adjective_type_rescaled %in% c("Negated positive", "Antonym")),
           position = position_dodge(width = 0.02), size = 10, inherit.aes = F,
           aes(x = condition_numeric,
                      y = mean,
...
```{r expt-results, fig.cap="Empirical ratings for adjective sets with morphological antonyms opposites (e.g., \"sad\") and antonyms (e.g., \"unhappy\"). A-B: Expts. 1, 2, respectively; participants rated all adjective types for both negative and antonym opposites. C: Four between-participants conditions. Left end-points of rating scales were defined by the opposite type (negative vs. antonym).", fig.env = "figure*", fig.pos = "h", fig.width=8, fig.asp = 0.6, out.width = "100%", fig.align = "center", set.cap.width=T, num.cols.cap=2}
...
fig.expt2.multiple <- d.full.boot %>%
  ...
  ggplot(., aes(x = 0,
    ...
    xlab("")+
    coord_flip()+
    geom_linerange(position = position_jitterdodge(dodge.width = 0.3, jitter.height = 0, jitter.width = 0), size = 10, alpha = 1)+
    geom_text(x = -(0.3/4) + 0.05, y = 0.1, label = 'lexical antonyms', color = 'darkgreen', size = 2.5)+
    geom_text(x = (0.3/4) + 0.05, y = 0.1, label = 'morphological antonyms', color = 'black', alpha = 0.6, size = 2.5)+
    geom_text(x = -(0.3/4) - 0.07, y = 0.7, label = 'multiple utterances', color = 'black', alpha = 0.8, size = 3)+
    geom_vline(xintercept =  -(0.3/4),  size = 5, alpha = 0.1, color = 'darkgreen')+
    ...
    scale_color_manual(name="Adjective type",
                    values = orange.purple.blue.color.palette,
                    guide = guide_legend(keywidth = 0.01, keyheight = 0.01, nrow = 2, reverse=F),
                    breaks = c("Lexical antonym", "Negated lexical antonym", 
                               "Morphological antonym", "Negated morphological antonym", 
                               "Positive", "Negated positive"))+
    ylab("mean normalized rating")+
