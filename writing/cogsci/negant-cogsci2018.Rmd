...
To better understand this model, we consider three cases whose qualitative predictions are displayed in Figure$\thinspace$\ref{fig:modelPredictions}.^[These model predictions assume the following model parameters: $P(x) = \mathcal{N}(0, 1); \alpha = 1; \text{cost}(\mathit{un}) = 1; \text{cost}(\mathit{not}) = 2$.]
...
This model does not have lexical uncertainty about overt negation markers and, consequently, does not differentiate between negated antonym *not unhappy* and positive *happy*, nor negated positives and antonyms.

The *bonafide contraries model* either assumes that antonyms (*unhappy*) convey contrary opposition, while maintaining uncertainty about the meaning of particle negation (*not happy*).
...
That is, the *bonafide contraries* model predicts a full ordering: *unhappy* $\approx$ *not happy* $<$ *not unhappy* $<$ *happy*.
