...
\mht{i think we could get a prediction for expt 2 / expt 3 (expl. vs.\text{~}impl. alternatives) by conditioning the uncertain parsing model on all 4 (or at least more than 1) utterance, while keeping the lexicon fixed. for instance, to interpret *not unhappy*, the listener must ascribe *not* as logical negation and *un-* as contrary... if the listener must fit all the utterances to the same lexicon, that should make it more like the bonafide antonyms model... \mf{YES!}}
...
\mf{I have not attempted to iron in the new terminology you suggested here; this part could be more in line with the previous, but you may have a clearer/different idea, so I leave this to you.}

The *full uncertainty model* predicts that affixal negation (*unhappy*) and particle negation (*not happy*) receive similar interpretations, while the negated antonym (*not unhappy*) are interpreted more weakly than the simple positive form (*happy*).
We contrast this quasi-ordering for *morphological antonyms* (*unhappy* $\approx$ *not happy* $<$ *not unhappy* $<$ *happy*) with a full-ordering predicted for *lexical antonyms* (*short* $<$ *not tall* $<$ *not short* $<$ *tall*) by the *bonafide antonyms* model. 
...
Participants were asked to rate the character on a scale from "the most \textsc{POS} person" to "the most \textsc{ANT} person", using a slider bar (\figref{fig:experiment-slides}A).
...
The predictions of our models concern the nature of the ordering of terms within an adjective set (i.e., POS vs.\text{~}ANT vs.\text{~}NEG ANT vs.\text{~}NEG POS).
...
As predicted by the *bonafide antonyms model*, lexical antonyms show a strict ordering: $\textsc{ant} < \textsc{neg pos} < \textsc{neg ant} < \textsc{ant}$.
Furthermore, as predicted by the *full uncertainty model*, morphological antonyms show only a partial ordering, critically with antonyms and negated positives getting identical interpretations, consistent with the intuitions of @Jespersen1917:Negation and @Blutner2004:pragmatics.
...
This experiment tests the effect on interpretation of presenting simultaneously and explicitly a set of alternative utterances. 
