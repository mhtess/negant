...
To evaluate this hypothesis, we built a mixed-effects regression model with by-participant and by-item random effects of intercept and adjective type (i.e., POS vs. ANT vs. NEG ANT vs. NEG POS).^[
  This model was the maximal mixed-effects model that converged for our data set. 
  In addition, it explained significantly more variance than models with slightly reduced mixed-effects structures.
]

negations will be different (lexical antonyms &lt; negations, in terms of the slider ratings with the above defined scale) whereas morphological antonyms and negations should not show a difference (morphological ~= negation). Given our design, we register the following mixed-effects regression model:
