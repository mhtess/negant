...
A straight-forward, compositional analysis, however, would map morphological affixes (*un-*, *im-*) and negation particles (e.g., *not*) to proposition-level negation ($\neg$) in modern standard logic. 
...
Alternatively, *morphological antonyms* like *unhappy* could behave like words with contrary meanings (e.g., *tall* and *short*).
Contraries cannot both be true but they can both be false [@Horn1989:Natural]: Jones may be neither tall nor short. 
...
*Not unhappy* would be pragmatically strengthened in context to have a more specific than literally the negation of *unhappy*.
...
The meanings of gradable adjectives can be modeled using a threshold function: $\denote{happy} = \lambda e \thinspace . \thinspace \text{happiness}(e) > \theta$, whose threshold variable $\theta$ is supplied by the context [@Kennedy2007].


<!-- while maintaining structural constraints on composition. -->
...
Interestingly, the *uncertain negation* model does not differentiate antonyms (*unhappy*) from negated positives (*not happy*)---in line with the intuition of @Jespersen1917:Negation and @Blutner2004:pragmatics.
...
An alterantive *pure logical negation* (or *George Orwell*) can be derived by assigning 0 probability to lexica with contrary interpretations of overt negation markers. 
...
As a second alternative, we consider a model that interprets morphological negation (*un*) as expressing true contrary opposition by assigning 0 probability to lexica with contradictory interpretations of morphological antonyms.
...
All models have more exaggerated interpretations from utterances.
The *uncertain negation* model here qualitatively differentiates itself from the single utterance simulation: It now predicts a true ordering as in Figure \ref{fig:happy-scale} between negated positives and antonyms,.
