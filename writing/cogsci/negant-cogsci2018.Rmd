...
*Not unhappy*, then, would compete as pragmatic alternative to *happy* (a speaker could use to describe a state of affairs) and *not unhappy* would be pragmatically strengthened in context to yield an interpretation more specific than its literal meaning.
...
Our model interprets *not unhappy* as a slightly positive state and predicts qualitative differences between morphological antonyms (e.g., \{*happy*, *unhappy*\}) and lexical antonyms which lack overt negation markers (\{*tall, short*\}).
These differences are shown to be sensitive to the presence of explicit alternative utterances [as illustrated in @Krifka2007:Negated-antonyms's example above]. 
...
```{r lexicon-model, fig.pos = "b", fig.cap="Hypothesis space of lexica for the uncertain negation model. The hypothesis space is restricted to the interpretations that are circled for the logical negation and bonafide contraries model."}
# cowplot::ggdraw() +  
...
Possible lexica differ in how they semantically parse overt negation markers (*un-*, *not*; Fig.$\thinspace$\ref{fig:lexicon-model}).
<!-- According to the *uncertain negation* model, listeners have uncertainty about which lexicon a speaker may be using; -->
We explore the simplest instantiation of this *uncertain negation* model, putting equal probability on the logically possible parses of utterances with negation.
Upon hearing *not unhappy*, the model reasons that a truly compositional $\neg \neg \text{happy}$ is implausible (intuitively because the speaker could have said the simpler *happy*) and interprets the utterances as signalling a slightly positive state, via pragmatic competition with the other utterances (Fig.$\thinspace$\ref{fig:modelPredictions}; single utterance, *uncertain negation*, light purple paddle).^[Model predictions assume the following model parameters: $P(x) = \mathcal{N}(0, 1); \alpha = 1; \text{cost}(\mathit{un}) = 1; \text{cost}(\mathit{not}) = 2$.]
Interestingly, the *uncertain negation* model does not differentiate antonyms (*unhappy*) from negated positives (*not happy*), as @Jespersen1917:Negation and @Blutner2004:pragmatics surmised.

We generate alternative hypotheses by perturbing the lexicon prior $P(\mathcal{L})$.
A pure *logical negation* (or *George Orwell*) alternative model can be derived by assigning probability zero to lexica with contrary interpretations of negation. 
This model believes negation markers only express contradictory opposition (Fig.$\thinspace$\ref{fig:lexicon-model}, orange, solid). 
...
As a second alternative, we consider a model that interprets morphological negation (*un-*) as expressing true contrary opposition by assigning probability zero to lexica with contradictory interpretations of morphological antonyms (Fig.$\thinspace$\ref{fig:lexicon-model}, purple, dotted).
This *bonafide contraries* hypothesis still has uncertainty about how to interpret particle negation (*not*), but in context, reasons that *not* should be interpreted as contradictory opposition ($\neg$) so as to keep the lexicon non-redundant.
This model interprets negated antonyms (*not unhappy*) as slightly positive and predicts the ordering hypothesized by @Krifka2007:Negated-antonyms and shown in Fig.\ref{fig:happy-scale} (single utterance).

Our models resolve the meaning of negation in context. 
As such, multiple utterances heard in the same context can help resolve the meaning of each, and thus yield different interpretations than if the utterances are heard in isolation.
...
The *uncertain negation* model, though, uniquely differentiates itself from the single utterance simulation: It now predicts a difference between negated positives and antonyms, and the ordering in Fig.$\thinspace$\ref{fig:happy-scale} (Fig.$\thinspace$\ref{fig:modelPredictions}; multiple utterances, *uncertain negation*).
...
The *uncertain negation model* predicts that overt negation markers (*un-*, *not*) come with uncertainty in meaning (contrary vs. contradictory negation) that produces a partial ordering for antonyms and their negations when heard in isolation, but a full ordering when present in the same context (Fig.$\thinspace$\ref{fig:modelPredictions}, single vs. multiple utterances, *uncertain negation*).
As a control condition, we examine antonyms which do not have overt negation markers (e.g., *short*); we predict they will behave like the *bonafide contraries* simulation and display a full ordering regardless of context. 
...
\text{Expt.$\thinspace$2} is a more powerful and more stringent, preregistered (\url{osf.io/p7f25/}) confirmatory replication.
...
```{r modelPredictions, fig.cap="Predictions for the strengths of adjective interpretations for three model variants.", fig.env = "figure", fig.pos = "t", fig.width=3.5, fig.align = "center"}
load('cached_results/rsa_model_predictions.RData') # rs.listener.wp.expectation
...
ggplot(rs.listener.wp.expectation %>%
           filter(!(src == "logical negation" | (src == "uncertain negation" & utterance %in% c("Negated\npositive", "Antonym") & 
    ...
    geom_rect(data = rs.listener.wp.expectation %>%
           filter(src == "logical negation", utterance %in% c("Negated\npositive", "Positive")),
    ...
    geom_rect(data = rs.listener.wp.expectation %>%
           filter(src == "logical negation", utterance %in% c("Negated\nantonym", "Antonym")),
             inherit.aes = F, aes(xmin = src_numeric-0.025, xmax = src_numeric+0.1,
                       ...
                       fill = utterance), color = 'black')+
    geom_rect(data = rs.listener.wp.expectation %>%
           filter(src == "uncertain negation" & utterance == "Negated\npositive", utterances == "single utterance"),
             inherit.aes = F, aes(xmin = src_numeric-0.1, xmax = src_numeric+0.025,
                       ymin = interpretation-0.05, ymax = interpretation+0.05,
                       fill = utterance), color = 'black')+
    geom_rect(data = rs.listener.wp.expectation %>%
...
We used adjectives that described properties of people that fell into four forms: a positive adjective, its antonym (morphological or lexical), and their negations using the particle "not". 
We refer to a collection of these four with a particular positive adjective as an *adjective set* (e.g., one adjective set is *happy*, *unhappy*, *not happy*, *not unhappy*).
...
We additionally observe that negated morphological antonyms (e.g., *not unhappy*) were rated lower than negated lexical antonyms (e.g., *not tall*; \figref{fig:expt-results}A). 
...
```{r experiment-slides, fig.pos = "hbt", fig.align ="center", fig.cap="Example experimental trials for (A) single utterance (Expts. 1, 2) and (B) multiple utterances conditions (Expt. 2). \``in the world\'' wording for endpoints was used in Expt. 2."}
...
In this experiment, we aim to replicate the asymmetry findings using adjectives that describe the same semantic scales.
...
In addition, we test our second prediction that hearing multiple utterances in the same context will produce the full ordering for morphological antonym sets (Fig.$\thinspace$\ref{fig:modelPredictions}).
...
The *single utterances* conditions were similar to that \text{Expt.$\thinspace$1}: Participants rated one sentence at a time (e.g., "Greg is not unhappy"), each from a unique adjective set (e.g., never rated both *unhappy* and *not happy*), completing a total of 12 trials, with exactly 3 repetitions of each adjective type (positive, antonym, and their negations).
...
To evaluate this hypothesis, we considered data only from the morphological antonyms conditions and built a linear mixed model predicting the raw, unnormalized ratings in terms of fixed effects of adjective type,
...
This work resolves an outstanding puzzle in natural language understanding: How to interpret double negatives.
...
Here, we develop a theory of interpretation for negated antonyms by introducing logical negation into a pragmatic model of adjective interpretation. 
But morphological antonyms (*unhappy*) do not behave like bonafide antonyms
We discovered and confirmed a surprising empirical result, which both validates and challenges existing intuitions in linguistics: *unhappy* and *not happy* are not immediately differentiated, except when both are present in the same context.
A model of fixed meanings for negation cannot account for this pattern of data.
However, a model that represents uncertainty about how to parse overt negation markers (*un-*, *not*) predicts this very result. 
While the items used in our experiment were all evaluative statements about people, we expect the asymmetries observed in our data to generalize to other negated antonyms.
...
This work thus carries with it an interesting account of a robust linguistic intuition: Potentially equivalent expressions receive differential interpretations when presented in direct co-text. \mht{what is co-text?} 
Reasoning about lexical ambiguity, listeners conclude that a choice of different expressions may be most likely for a speaker who differentiates meanings. \mf{you like this? more?} \mht{go on...}
...
\vspace{1em} \fbox{\parbox[b][][c]{7.3cm}{\centering Experimental paradigms, computational models, and data for this paper can be found at \url{https://mhtess.github.io}.
}} \vspace{1em}
