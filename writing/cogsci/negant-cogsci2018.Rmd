...
Any predicted qualitative differences between interpretations of positive adjectives (*happy*), antonyms (*unhappy*), negated positives (*not happy*) and negated antonyms (*not unhappy*) will depend on the space of meanings considered in the lexicon prior $P(\mathcal{L})$.
...
One possibility is that negation markers map only onto contradictory meanings (the *logical negation* or *George Orwell* class of meanings; Fig.$\thinspace$\ref{fig:lexicon-model} orange solid lines).
...
For maximal similarity to the full *uncertain negation* model, we allow the *bonafide anotnyms* model to maintain uncertainty about particle negation (*not*).^[
The alternative of fixing the meaning of particle negation (*not*) to be logical negation turns out to not produce any qualitative difference from maintaining uncertainty about the meaning, in this model variant. The model infers that *not* should be treated as logical negation, given the other constraints in the lexicon.
]
...
Model predictions assume the following minimally assumptive model parameters: $P(x) = \mathcal{N}(0, 1); \alpha = 1; \text{cost}(\mathit{un}) = 1; \text{cost}(\mathit{not}) = 2$.^[
...
300 participants were assigned to each *anotnym type* in the *single utterance* contexts, and 75 participants were assigned to each in the *multiple utterances* conditions.
