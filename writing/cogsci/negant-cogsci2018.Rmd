...
Even if we reject Orwell and consider "unhappy" as an antonym to "happy", the mechanism by which "not unhappy" gets an interepretation is less then clear [@Krifka2007:Negated-antonyms].
Pragmatic competition with "happy" is surely part of the solution, but the simplest implementation of this idea [@Horn1991:Duplex; @Horn1993:Economy] renders "not unhappy" as meaning *neutral* or *indifferent*, contrary to the intuition that "not unhappy" is a slightly positive state.
"Not happy" could also be an alternative that competes with "not unhappy" [@Blutner2004:pragmatics], but how pragmatics contextually strengthens "not happy" has not been fully spelled out [@Krifka2007:Negated-antonyms].
Furthermore, intuitions about differences in meaning between "not happy" and "unhappy" are (shall we say) not uncontroversial: @Jespersen1917:Negation and @Blutner2004:pragmatics argue that the two are basically identical in meaning, while @Horn1989:Natural and @Krifka2007:Negated-antonyms disagree.
...
We take a radical (but hopefully not unreasonable) starting point: meanings of negated gradable adjectives (including particle negations like "not happy") are contextually constructed by pragmatic reasoning about alternative utterances a speaker might have chosen. 
Our model comprises the possibility that a speaker might treat *affix negation* (e.g., "un-", "in-") as standard two-valued logical negation just as they might with *particle negation* "not". 
However, for each kind of negation, the speaker could assign independent thresholds, not derived compositionally from logical negation, to either particle- or affix-negated expressions. 
A pragmatic listener then uses Bayesian inference, based on the assumption of an informative cooperative speaker, to infer which theory of the speaker's use of particle and affix negation best rationalizes a given utterance.
...
The speaker model $S_1$  is an approximately rational Bayesian agent trying to inform a naive listener $L_0$ (Eq. \ref{eq:L0}) about the state of the world $x$.
...
We posit that listeners have uncertainty about the correct *parse* of affixal and particle negation, represented as a conditional probabilistic distribution over parses given different utterances: $P(r \mid u)$, where $r$ is a truth-conditional expression denoting the semantics of an utterance $u$. 
...
This hypothesis requires further elaboration of the RSA model to resolve the threshold for the bonafide antonym and specify the prior probability of parsing "not" or "un-" compositionally vs. noncompositionally. 
...
\vspace{-0.5cm}
\begin{align}
L_{1}(x, \theta_+, \theta_- \mid u) &\propto S_{1}(r \mid x, \theta_+, \theta_-)  P(x)  P(\theta_+)  P(\theta_-) P(r \mid u) \label{eq:L1ant}
\end{align}

\red{Figure X} show the pragmatic listener's (Eq. \ref{eq:L1ant}) interpretations of antonyms and their negations for hypothetical uniform priors over the degree $x$. 
The model predicts the quasi-ordering: "unhappy" $\approx$ "not happy" $<$ "not unhappy" $<$ "happy" (\red{Figure X (inset)}).
Even though the listener is uncertain about the parse for "unhappy" and "not happy", when it hears "not unhappy", it reasons that a truly compositional, double negation is unlikely, given the cost of the utterance. 

As a special case of this model, we can examine the listener interpretations when there is no uncertainty about the correct parse of utterances with negation, which we take as model of antonym pairs with unique lexical entries (e.g., "happy", "sad").
In this casem the alternative set is $\{\text{positive},  \text{negated positive}, \text{antonym}, \text{negated antonym}\}$, or $\{\text{POS},  \text{NEGPOS}, \text{ANT}, \text{NEGANT}\}$ for short. 
*Bonafide* antonyms receive their own independent threshold: $\denote{\text{ANT}} = x < \theta_-$, while modifier negation is logical negation $\denote{\text{NEGPOS}} = \neg \denote{\text{POS}}$.
As a final model variant, we consider the case where the listener parses negation only compositionally (i.e., the "George Orwell model"). 

\red{Figure Y} shows listener intrepretations for this set of utterances, without uncertainty about the parse.
Distinct from the uncertain parse model, this *bonafide antonyms* model predicts a full ordering: "sad" $\approx$ "not happy" $<$ "not sad" $<$ "happy".
We design our behavioral experiments to test the predictions of the *uncertain parse* and the *bonafide antonyms* models. 
In particular, we hypothesize that supposed antonyms created by affixal negation have uncertainty in parsing and thus will produce the partial ordering observed in \red{Figure X}.
Lexically distinct antonyms, on the other hand, we predict will exhibit a complete ordering. 
...
The *uncertain parse* model predicts that morphological negation (e.g., "unhappy") and particle negation (e.g., "not happy") will receive similar interpretations, while the negated antonym (e.g., "not unhappy") will be interpreted more weakly than the simple positive form (e.g., "happy").
...
Items were constructed from an informal survey of the linguistics literature and taken from list of ``common opposites'' available online\footnote{http://www.enchantedlearning.com/wordlist/opposites.shtml} (for a full list, see \tableref{tab:items1}).
...
The experiment comprised of four between-subjects experimental conditions arranged in a 2x2 Latin Square design: *antonym type (morphological vs. lexical)* X *alternatives (explicit vs. implicit)*, as described below.
...
```{r expt-results, fig.cap="Experimental results.", fig.env = "figure*", fig.pos = "h", fig.width=7, fig.height=4, fig.align = "center", set.cap.width=T, num.cols.cap=2}
load(file = "cached_results/bootstrappedCIs.RData")
 
fig.bs.ci <- d.full.boot %>%
  ungroup() %>%
  mutate(antonym_type = factor(antonym_type, levels = c(  "morphant", "lexant"),
         ...
         adjective_type_rescaled = factor(adjective_type_rescaled,
                                          levels = c( "positive",
                                                ...
                                                "antonym"),
                                          labels = c("Positive", 
                                                     "Negated antonym","Negated positive","Antonym"))) %>%
...
plot_grid(
  cowplot::plot_grid(
    ...
    nrow = 2),
  labels = c("", "C"),
  fig.bs.ci, ncol = 2, rel_widths = c(1,2))
