...
Our model comprises the possibility that a speaker might treat both *affix negation* (e.g., "un-", "in-") and *particle negation* (e.g., "not") as standard two-valued logical negation. 
...
The speaker model $S_1$  is an approximately rational Bayesian agent (with degree of rationality $\alpha$) trying to inform a naive listener $L_0$ (Eq. \ref{eq:L0}) about the state of the world $x$.
...
\vspace{-0.5cm}
\begin{align}
L_{1}(x, \theta_+, \theta_- \mid u) &\propto S_{1}(u \mid x, \theta_+, \theta_-, \mathcal{L})  P(x)  P(\theta_+)  P(\theta_-) P(\mathcal{L}) \label{eq:L1ant}
\end{align}

\mht{should we write eq. with marginalization over lexica / thresolds?}

As a special case of this model, we can examine the listener interpretation when there is no uncertainty about the meaning of an anotnym, which we take to be the case when a distinct lexical item has an opposing meaning (e.g., "sad" for "happy").
*Bonafide antonyms* receive their own independent threshold: $\denote{\text{ANT}} = x < \theta_-$, while uncertainty remains in this model about the meaning of particle negation (either it is an antonym and receives its own threshold or is parsed as logical negation).
As a final model variant, we consider the case where the listener's lexicon treats negation only compositionally (i.e., the "George Orwell model"). 

Figure \ref{fig:modelPredictions} shows listener interpretations of antonyms and their negations for each of the three model variants. 
These model predictions assume the following model parameters: $P(x) = \mathcal{N}(0, 1); \alpha = 1; \text{cost}(``un") = 1; \text{cost}(``not") = 2$, as well as equal probablility of lexica with compositional vs. noncompositional interpretations of negation when appropriate (depending on model variant). 
The model based on the intuition of George Orwell (*logical negations*) does not differentiate between negated antonyms and positive statementes, nor negated positives and antonyms.
The model that assumes only lexica that ascribe independent thresholds to antonyms (*bonafide antonyms*) provides weaker interpretations to negated antonyms than to positives, and weaker interpretations of negated positives than to antonyms, consistent with the prediction of @Krifka:negated-antonyms. 
...
Finally, the *uncertain parser* model predict a quasi-ordering: "unhappy" $\approx$ "not happy" $<$ "not unhappy" $<$ "happy" (\red{Figure X (inset)}).
Even though the listener is uncertain about the parse for "unhappy" and "not happy", when it hears "not unhappy", it reasons that a truly compositional, double negation is unlikely. 
We design our behavioral experiments to test the predictions of the *uncertain parse* and the *bonafide antonyms* models. 
In particular, we hypothesize that supposed antonyms created by affixal negation have uncertainty in parsing and thus will produce the partial ordering observed in Figure \ref{fig:modelPredictions}.
