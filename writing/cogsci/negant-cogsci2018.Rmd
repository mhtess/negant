...
Furthermore, as predicted by the *full uncertainty model* and consistent with the intuitions of @Jespersen1917:Negation and @Blutner2004:pragmatics, adjective sets with *antonyms* show only a partial ordering: $\textsc{opposite}_\textsc{ant} \approx  \textsc{negated positive} < \textsc{negated opposite}_\textsc{ant} < \textsc{positive}$.
...
We additionally observe that interpretations of negated antonyms (e.g., *not unhappy*) were appreciably weaker for than those of negated negatives (e.g., *not tall*).
...
```{r experiment-slides, fig.pos = "b", fig.cap="Example experimental trials.", fig.env = "figure*", fig.width=7, fig.height=1.25, fig.align = "center", set.cap.width=T, num.cols.cap=2}
...
The procedure differed in that participants saw and rated all four adjective types simultaneously (\figref{fig:experiment-slides}B). 
In addition, the endpoints of the slider bars were relabeled to "the most \{\textsc{positive}, \textsc{opposite}\} person *in the world*"; without "in the world", there is a salient interpretation of the endpoints as indicating  "the most \{\textsc{positive}, \textsc{opposite}\} person (of these four)".
...
We again find a subset of responses to the negated antonym (*not unhappy*) are appreciably below the midpoint, suggesting again a small effect of politeness on interpretation. 


## Experiment 3: Large-scale, within-item replication

\text{Expts$\thinspace$1 \& 2} revealed an asymmetry between opposites as negative adjectives and as antonyms.
Whereas negatives (e.g., "short") are clearly distinguished from negated positives (e.g., "not tall"), antonyms were not unless the alternative utterances were explicitly provided. 
In this experiment, we aim to replicate these findings while controlling for the semantic scale under consideration. 
Specifically, in \text{Expts.$\thinspace$1~\&~2}, our items varied both in terms of how opposites (negatives vs.\text{~}antonyms) as well as the actual degree scales being described (e.g., height for *tall*/*short* vs.\text{~}happiness for *happy*/*unhappy*).
...
To best isolate the contribution of antonyms vs.\text{~}negatives, we curated *adjective sets* consisting of words for properties of people, such that antonyms and negatives existed for the same positive adjective (e.g., *happy* $\rightarrow$ *unhappy*, *sad*; Table$\thinspace$3).
Negative adjectives were selected from a set of possibilities produced from a small survey (n=18) on MTurk eliciting "opposites" for a list of 30 positive-form adjectives, which had antonyms (asking participants in the same experimental context as our interpretation studies, "What is the opposite of e.g., *forgiving*?").
...
Ten out of the original 30 items were dropped for not having such a well-suited negative adjective (e.g., *moral*) or for having a well-suited negative that conflicted with another item (e.g., *compassionate* $\rightarrow$ *cold*, but also *affectionate* $\rightarrow$ *cold*).
...
print(tab1, type="latex", comment = F, table.placement = "b", size="\\fontsize{9pt}{10pt}\\selectfont", include.rownames=FALSE)
```
...
In the *implicit alternatives* conditions, participants rated one sentence at a time (e.g., *Greg is not unhappy*), each from a unique adjective set (e.g., never rated both *unhappy* and *not happy*), completing a total of 12 trials, with exactly 3 repetitions of each adjective type (\textsc{positive}, \textsc{opposite}, and their negations).
...
As we did in \text{Expt.$\thinspace$1}, we evaluate our hypothesis that negatives behave like the *bonafide contraries* model (i.e., show a true ordering) and affixal antonyms behave like the *full uncertainty* model (i.e., show a partial ordering). 
...
We hypothesized that uncertainty about the meaning of overt negation markers leads only to a partial ordering of affixal antonyms and their negations (e.g,. *not unhappy*). 
...
Our formalization of lexical uncertainty about the meaning of natural language negation builds on a growing movement in theories of language to treat the lexicon as not totally separable from the combinatorial rules of grammar [e.g., @bybee2006usage; @Odonnell2015productivity].
