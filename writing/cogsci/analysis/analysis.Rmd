...
d.items <- d.items[with(d.items, order(positive)),] %>%
  rename("Positive adjective" = positive, "Affixal antonym" = morphant,
         "Negative adjective" = lexant)

tab1 <- xtable::xtable(d.items, caption = "Adjectives used in Experiment 3.")
...
d.full.boot %>%
  ...
  # geom_density(data = d.full.natEng, 
  #              inherit.aes = F,
  #              )
  ...
  coord_flip()+
    ...
    size = 5)+
    geom_vline(xintercept = 1.2,  size = 5, alpha = 0.2, inherit.aes = F)+
    # geom_errorbarh(position = position_jitterdodge(dodge.width = 0.75,
    #                                 jitter.height = 0,
    #                                 jitter.width = 0.2),
    # #position = position_dodge(0.5),
    # size = 5)+
  ...
  #geom_ribbon(ymin=0.5, ymax=0.7, x=1, inherit.aes = F, fill = 'black',alpha = 1)+
