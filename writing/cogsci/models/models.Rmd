...
var listener1 = cache(function(list_of_utterances) {
  Infer({model: function(){
    ...
    return extend(lexicon, {states: _.fromPairs(_.zip(list_of_utterances, state))})
  }})
}, 10000);
'
```
...
// _.fromPairs(
//  ["not_happy", marginalize(marginalize(post, "states"), "not_happy")],
//  ["unhappy", marginalize(marginalize(post, "states"), "unhappy")]
// )
marginalize(post, "states")
'
```
...
rs.listener.wp <- webppl(paste(rsaBinsCoarserGauss,
                                 ...
                                 multipleUtterancesListenerCall,
                                 sep = '\n'),
                           data = list(compositional_un = 0.5, compositional_not = 0.5), 
...
# model.variants in terms of prior probabilities of different lexica
    # {un: "compositional", not: "compositional"},
    # {un: "noncompositional", not: "compositional"},
    # {un: "compositional", not: "noncompositional"},
    # {un: "noncompositional", not: "noncompositional"}
model.variants = list(
  "uncertain parser" = list(compositional_un = 0.5, compositional_not = 0.5),
  "bonafide antonyms" =  list(compositional_un = 0, compositional_not = 0.5),
...
for (modelName in names(model.variants)){
  ...
  rs.listener.wp.expectation <- bind_rows(
    ...
    rs.listener.wp.tidy.temp %>%
      group_by(utterance) %>% summarize(interpretation = sum(probs * support)) %>%
      mutate(src = modelName)
  )
  
  rs.listener.wp.samples <- bind_rows(
    rs.listener.wp.samples, 
    get_samples(rs.listener.wp.tidy.temp %>% rename(prob = probs), 10000) %>% mutate(src = modelName)
  )
}
